...
class WindowCursorRect < Rect
  ...
  def set(x, y, width, height)
    ...
    begin
      ...
      self.x = x rescue nil
      self.y = y rescue nil
      self.width = width rescue nil
      self.height = height rescue nil
    end
    @window.width = @window.width rescue nil
  end
  ...
  def ensureBitmap(bitmap, dwidth, dheight)
    if !bitmap || bitmap.disposed? || bitmap.width < dwidth || bitmap.height < dheight
      ...
      begin
        ...
        bitmap = Bitmap.new(32, 32) rescue nil
      end
    end
    return bitmap
  end
  ...
  def privRefresh(changeBitmap = false)
    ...
    if changeBitmap && @_windowskin && !@_windowskin.disposed?
      ...
      4.times do |i|
        ...
        if @sidebitmaps[i] && !@sidebitmaps[i].disposed?
          @sprites["side#{i}"].bitmap = @sidebitmaps[i]
          @sprites["side#{i}"].src_rect.set(0, 0, dwidth, dheight)
          @sidebitmaps[i].clear rescue nil
          if sideRects[i].width > 0 && sideRects[i].height > 0
            @sidebitmaps[i].stretch_blt(@sprites["side#{i}"].src_rect, @_windowskin, sideRects[i]) rescue nil
          end
        end
      end
      backwidth = @width - 4
      ...
      if backwidth > 0 && backheight > 0
        ...
        if @backbitmap && !@backbitmap.disposed?
          @sprites["back"].bitmap = @backbitmap
          @sprites["back"].src_rect.set(0, 0, backwidth, backheight)
          @backbitmap.clear rescue nil
          if @stretch
            @backbitmap.stretch_blt(@sprites["back"].src_rect, @_windowskin, backRect) rescue nil
          else
