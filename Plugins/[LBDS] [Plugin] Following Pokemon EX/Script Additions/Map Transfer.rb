...
module FollowingPkmn
  ...
  def self.hide_follower
    ...
    if FollowingPkmn.get_event
      FollowingPkmn.get_event.transparent = true rescue nil
      FollowingPkmn.get_event.opacity = 0 rescue nil
    end
  end
  ...
  def self.unhide_follower
    ...
    @@hidden = false
    if FollowingPkmn.get_event
      FollowingPkmn.get_event.transparent = false rescue nil
      FollowingPkmn.get_event.opacity = 255 rescue nil
    end
    # Check if follower should be active (without hidden check)
    if ($PokemonGlobal&.follower_toggled != false) && $player&.first_able_pokemon
      self.refresh(false) rescue nil
    end
  end
end
#-------------------------------------------------------------------------------
