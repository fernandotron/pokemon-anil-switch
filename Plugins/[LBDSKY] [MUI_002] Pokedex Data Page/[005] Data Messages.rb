...
class PokemonPokedexInfo_Scene
  ...
  def pbDrawDataNotes(cursor = nil)
    ...
    case cursor
    when :encounter then
      text = pbDataTextEncounters(path, species, overlay)
      pbDrawTextPositions(overlay, [[_INTL("Pulsa [C] para entrar en"), Graphics.width - 34, 292, :right, Color.new(0, 112, 248), Color.new(120, 184, 232)]])
      pbDrawTextPositions(overlay, [[_INTL("las opciones y muévete"), Graphics.width - 34, 292+32, :right, Color.new(0, 112, 248), Color.new(120, 184, 232)]])
      pbDrawTextPositions(overlay, [[_INTL("con las flechas."), Graphics.width - 34, 292+32+32, :right, Color.new(0, 112, 248), Color.new(120, 184, 232)]])
  ...
  def pbDataTextGeneral(path, species, overlay, inMenu = false)
    ...
    if owned
      ...
      pbDrawTextPositions(overlay, [
        [_INTL("[C]: Relacionados"), Graphics.width - 34, 292, :right, Color.new(0, 112, 248), Color.new(120, 184, 232)]
  ...
  def pbDataTextItems(path, species, overlay)
    ...
    if owned
      ...
      if nil_or_empty?(text)
        ...
        pbDrawTextPositions(overlay, [
          [_INTL("[C]: Más información"), Graphics.width - 34, 292, :right, Color.new(0, 112, 248), Color.new(120, 184, 232)]
        ])
      end
    else
      text << "Desconocido."
  ...
  def pbDataTextStats(path, species, overlay, s2 = nil)
    ...
    if owned
      ...
      @api_data = PokeAPI.get_data(species) if !s2 && (!@api_data || @api_data["species"] != species.id || @api_data["form"] != @form)
  ...
  def pbDataTextEggGroup(path, species, overlay, s2 = nil)
    ...
    if owned
      ...
      pbDrawTextPositions(overlay, [
        [_INTL("[C]: Pokémon compatibles"), Graphics.width - 34, 292, :right, Color.new(0, 112, 248), Color.new(120, 184, 232)]
  ...
  def pbDataTextFamily(path, species, overlay, inMenu = false)
    ...
    if nil_or_empty?(text)
      ...
      #-------------------------------------------------------------------------
      if [:PICHU_2, :FLOETTE_5].include?(species.id) ||
         species.species == :PIKACHU && (8..16).include?(species.form)
      ...
      #-------------------------------------------------------------------------
        ...
        form = 2 if species.id == :DARMANITAN_3
        ...
        # else
          ...
          evolutions = evolutions.select { |evo| evo[0] == species.species && evo[1] != :None }
          ...
          evolutions.each do |evo|
            ...
            show_form_name = spec == :GIMMIGHOUL ? false : true
            text << data.description(spec, evo[0], evo[2], nil_or_empty?(text), show_form_name, t)
            break if index > count - 1
            index += 1
          end
          ...
          # Add a single dot at the end of all evolution methods
          text += "." if !text.empty? && !text.end_with?(".")
        # end
        #-----------------------------------------------------------------------
      ...
      if !inMenu && !@data_hash[:family].empty? #&& $player.owned?(@species)
        pbDrawTextPositions(overlay, [
          [_INTL("[C]: Evoluciones"), Graphics.width - 34, 292, :right, Color.new(0, 112, 248), Color.new(120, 184, 232)]
        ])
      end
    end
    return text
