class Battle
  ...
  def pbStartBattleSendOut(sendOuts)
    ...
    if wildBattle?
      ...
      when 1
        pbDisplayPaused(_INTL("¡Un {1} salvaje te corta el paso!", foeParty[0].name.force_encoding("UTF-8")))
      when 2
        pbDisplayPaused(_INTL("¡Un {1} y un {2} salvajes te cortan el paso!", foeParty[0].name.force_encoding("UTF-8"),
                              foeParty[1].name.force_encoding("UTF-8")))
      when 3
        pbDisplayPaused(_INTL("¡Un {1} , un {2} y un {3} salvajes te cortan el paso!", foeParty[0].name.force_encoding("UTF-8"),
                              foeParty[1].name.force_encoding("UTF-8"), foeParty[2].name.force_encoding("UTF-8")))
      end
    else   # Trainer battle
    ...
    [1, 0].each do |side|
      ...
      trainers.each_with_index do |t, i|
        ...
        case sent.length
        when 1
          msg += _INTL("¡{1} saca a {2}!", t.full_name, @battlers[sent[0]].name.force_encoding("UTF-8"))
        when 2
          msg += _INTL("¡{1} saca a {2} y {3}!", t.full_name,
                       @battlers[sent[0]].name.force_encoding("UTF-8"), @battlers[sent[1]].name.force_encoding("UTF-8"))
        when 3
          msg += _INTL("¡{1} saca a {2}, {3} y {4}!", t.full_name,
                       @battlers[sent[0]].name.force_encoding("UTF-8"), @battlers[sent[1]].name.force_encoding("UTF-8"), @battlers[sent[2]].name.force_encoding("UTF-8"))
        end
      ...
      if side == 0
        ...
        case sent.length
        when 1
          msg += _INTL("¡Adelante, {1}!", @battlers[sent[0]].name.force_encoding("UTF-8"))
        when 2
          msg += _INTL("¡Adelante, {1} y {2}!", @battlers[sent[0]].name.force_encoding("UTF-8"), @battlers[sent[1]].name.force_encoding("UTF-8"))
        when 3
          msg += _INTL("¡Adelante, {1}, {2} y {3}!", @battlers[sent[0]].name.force_encoding("UTF-8"),
                       @battlers[sent[1]].name.force_encoding("UTF-8"), @battlers[sent[2]].name.force_encoding("UTF-8"))
        end
        toSendOut.concat(sent)
      end
  ...
  def pbStartBattleCore(battle_loop = true)
    # Fast preload of all participants' moves, cries, sprites and animations in RAM
    SwitchAssetOptimizer.prewarm_battle(self) if defined?(SwitchAssetOptimizer)
    SwitchPreloadManager.preload_battle_participants(self) if defined?(SwitchPreloadManager)
  ...
  def pbEndOfBattle
    ...
    @battlers.each do |b|
      ...
      b.pokemon.makeUnmega if b.mega? && b.isSpecies?(:ZYGARDE)
    end
