...
class Battle::Move::RecoilMove < Battle::Move
  def pbEffectAfterAllHits(user, target)
    ...
    return if user.hasActiveAbility?(:ROCKHEAD) || user.hasActiveAbility?(:ICEHEAD)
...
class Battle::Move::UseMoveTargetIsAboutToUse < Battle::Move
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    if !oppMove || oppMove.statusMove? || oppMove.powerMove? || @moveBlacklist.include?(oppMove.function_code)
      @battle.pbDisplay(_INTL("¡Pero falló!")) if show_message
      return true
    end
    return false
...
class Battle::Move::DestroyTargetBerryOrGem < Battle::Move
  def pbEffectWhenDealingDamage(user, target)
    ...
    target.pbRemoveItem
    @battle.pbDisplay(_INTL("¡El {1} de {2} ha sido incinerado!", item_name, target.pbThis(true)))
  end
end
