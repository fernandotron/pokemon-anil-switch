...
class Pokemon
  ...
  def calcHP(base, level, iv, ev)
    return 1 if base == 1
    iv = ev = 0 if Settings::DISABLE_IVS_AND_EVS || ($game_switches && $game_switches[149])
...
class Battle::Battler
  ...
  def pbCanInflictStatus?(newStatus, user, showMessages, move = nil, ignoreStatus = false)
    ...
    if (immunities.include?(:ALLSTATUS) || immunities.include?(newStatus)) && !self_inflicted && !ignoreStatus
      case newStatus
      when :SLEEP     then msg = _INTL("¡{1} es completamente inmune a ser dormido!", pbThis)
      when :POISON    then msg = _INTL("¡{1} es completamente inmune a ser envenenado!", pbThis)
      when :BURN      then msg = _INTL("¡{1} es completamente inmune a ser quemado!", pbThis)
      when :PARALYSIS then msg = _INTL("¡{1} es completamente inmune a ser paralizado!", pbThis)
      when :FROZEN    then msg = _INTL("¡{1} es completamente inmune a ser congelado!", pbThis)
      when :FROSTBITE then msg = _INTL("¡{1} es completamente inmune a ser congelado!", pbThis)
      when :DROWSY    then msg = _INTL("¡{1} es completamente inmune a ser adormecido!", pbThis)
      end
