    ...
    if arg1
      if arg2
        ...
        if defined?(RPG::Cache)
          self.bitmap = (RPG::Cache.load_bitmap("", arg1) rescue Bitmap.new(arg1))
        else
...
class SpriteHash
  ...
  def add(key, value)
    clear_disposed
    key = key.to_sym if key.respond_to?(:to_sym) && !key.is_a?(Numeric)
    @sprites[key] if @sprites[key] && @sprites[key].respond_to?(:dispose)
    @sprites[key] = value
    clear_disposed
  end
  ...
  def each
    clear_disposed
    @sprites.each { |s| yield s[1] if block_given? }
  end
  ...
  def dispose
    clear_disposed
    for key in @sprites.keys
      @sprites[key].dispose if @sprites[key].respond_to?(:dispose)
    end
    clear_disposed
  end
