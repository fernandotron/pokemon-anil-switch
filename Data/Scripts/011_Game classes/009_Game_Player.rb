...
class Game_Player < Game_Character
  ...
  def set_movement_type(type)
    ...
    when :cycling, :cycling_fast, :cycling_jumping, :cycling_stopped
      if !@move_route_forcing
        self.move_speed = (type == :cycling_jumping) ? 6 : 8
      end
  ...
  def update
    ...
    if (moving? || jumping?) && !@bumping
      if !@moved_last_frame || @stopped_last_frame || ($game_temp&.followers && $game_temp.followers.followers_need_move?)
...
def pbGetPlayerCharset(charset, trainer = nil, force = false)
  ...
  curr_name = $game_player&.instance_variable_get(:@character_name)
  force = true if $game_player && (curr_name.nil? || curr_name == "")
