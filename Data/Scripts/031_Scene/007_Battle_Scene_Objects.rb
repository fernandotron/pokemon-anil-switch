...
class Battle::Scene::PokemonDataBox < Sprite
  ...
  # Text colors
  NAME_BASE_COLOR         = Color.new(255,255,255) # Color.new(72, 72, 72)
  NAME_SHADOW_COLOR       = Color.new(75,75,75)    # Color.new(184, 184, 184)
  ...
  def initializeDataBoxGraphic(sideSize)
    ...
    when 2
      ...
      @spriteY += [-20, -39, 34, 25][@battler.index]
    when 3
  ...
  def initializeOtherGraphics(viewport)
    ...
    @contents = Bitmap.new(@databoxBitmap.width, @databoxBitmap.height + 20)
  ...
  def pbDrawNumber(number, btmp, startX, startY, align = :left)
    ...
    when -2
      ...
      begin
        num = number.to_i
        # Asegurar que el número esté en un rango válido
        num = [num, 0].max  # No permitir números negativos
        num = [num, 999999].min  # Limitar a 6 dígitos máximo
        num.digits.reverse
      rescue
        [0]  # Si hay cualquier error, mostrar 0
      end
    end
  ...
  def draw_name
    ...
    pbDrawTextPositions(self.bitmap, [[@battler.name, @spriteBaseX + 8 - nameOffset, 12, :left,
                                       NAME_BASE_COLOR, NAME_SHADOW_COLOR, :outline]]
    )
  end
  ...
  def draw_level
    ...
    pbDrawTextPositions(self.bitmap, [["Nv." + @battler.level.to_s, @spriteBaseX + offset, 12, :left,
                                       NAME_BASE_COLOR, NAME_SHADOW_COLOR, :outline]]
    )
  ...
  def draw_gender
    ...
    nameWidth = self.bitmap.text_size(@battler.name).width
    offset = nameWidth > 116 ? 140 : 126
    pbDrawTextPositions(self.bitmap, [[gender_text, @spriteBaseX + offset, 12, :left, base_color, shadow_color]])
  end
  ...
  def draw_shiny_icon
    ...
    onPlayerSide = @battler.index.even?
    despl_x = (onPlayerSide) ? 12 : -6
    pbDrawImagePositions(self.bitmap, [["Graphics/UI/shiny", @spriteBaseX + shiny_x + despl_x, 36]]) if !@battler.super_shiny?
    pbDrawImagePositions(self.bitmap, [["Graphics/UI/shiny_ur", @spriteBaseX + shiny_x + despl_x, 36]]) if @battler.super_shiny?
  end
  ...
  def update_positions
    ...
    if (@selected == 1 || @selected == 2) && BOBBING_DURATION   # Choosing commands/targeted
      bob_delta = System.real_uptime % BOBBING_DURATION   # 0-BOBBING_DURATION
...
class Battle::Scene::AbilitySplashBar < Sprite
  ...
  def refresh
    ...
    textPos.push([@battler.abilityName, textX, 8, align,
                  TEXT_SHADOW_COLOR, TEXT_BASE_COLOR, :outline])
    # Draw Pokémon's name
...
class Battle::Scene::BattlerSprite < RPG::Sprite
  ...
  def update
    ...
    if @selected == 1 && COMMAND_BOBBING_DURATION    # When choosing commands for this Pokémon
      bob_delta = System.real_uptime % COMMAND_BOBBING_DURATION   # 0-COMMAND_BOBBING_DURATION
    ...
    if @selected == 2 && @spriteVisible && TARGET_BLINKING_DURATION
      blink_delta = System.real_uptime % TARGET_BLINKING_DURATION   # 0-TARGET_BLINKING_DURATION
