...
module Battle::CatchAndStoreMixin
  ...
  def pbStorePokemon(pkmn)
    ...
    if @raidStyleCapture && !@caughtPokemon.empty?
      ...
      #pbDisplayPaused(_INTL("¡{1} ha sido enviado a la caja \"{2}\"!", pkmn.name, box_name))
    else
      dx_pbStorePokemon(pkmn)
    end
  end
...
class Battle::Battler
  def pbRaidStyleCapture(target, chance = nil, fleeMsg = nil, bgm = nil)
    ...
    @battle.pbDisplayPaused(_INTL("¡{1} está débil!\n¡Es el momento de capturarlo!", target.pbThis))
    @battle.scene.pbRevertBattlerStart
    @battle.scene.pbPauseScene(0.5)
    cmd = @battle.pbShowCommands(
      _INTL("¿Quieres capturar a {1}?", target.pbThis(true)), ["Capturar", "No capturar"], 1)
    pbPlayDecisionSE
    @battle.scene.pbRevertBattlerEnd
    case cmd
    when 0
      if defined?(MonotypeChallenge) && MonotypeChallenge.enabled? 
        is_valid, type_text = MonotypeChallenge.valid_monotype_with_text?(target.pokemon)
        if !is_valid
          @battle.pbDisplayPaused(_INTL("¡Solo puedes capturar a Pokémon de tipo {1}!", type_text))
      ...
      if $PokemonStorage.full?
        ...
        if ball
          ...
          if !chance.nil? && chance > 0
            ...
            if capture
              target.pokemon.perma_faint = false
              target.pokemon.hp = 1 if target.pokemon.hp == 0
              target.pokemon.form = get_legendary_capture_form(target.pokemon)
            end
          end
...
class Battle
  ...
  def pbItemMenu(idxBattler, firstAction)
    if @noBag
      pbDisplay(_INTL("No se pueden usar objetos en este combate."))
      return false
    end
