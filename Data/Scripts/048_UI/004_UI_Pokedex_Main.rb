...
class Window_Pokedex < Window_DrawableCommand
  ...
  def refresh
    ...
    self.contents.clear
    top = [self.top_item, 0].max
    bottom = [top + self.page_item_max, @item_max].min
...
#    end
  ...
  def pbGetDexList
    ...
    mode = $PokemonGlobal.pokedexMode
    cache_key = "#{region}_#{mode}_#{$player.pokedex.seen_count(region)}_#{$player.pokedex.owned_count(region)}"
    if @cached_dex_list && @cached_dex_key == cache_key
      return @cached_dex_list.clone
    end
    ...
    regionalSpecies.each_with_index do |species, i|
      next if !species
      next if !pbCanAddForModeList?(mode, species)
