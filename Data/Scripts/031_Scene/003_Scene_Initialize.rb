class Battle::Scene
  ...
  def pbStartBattle(battle)
    @battle   = battle
    SwitchAssetOptimizer.prewarm_battle(battle) if defined?(SwitchAssetOptimizer)
  ...
  def pbInitSprites
    ...
    if @battle.wildBattle?
      @battle.pbParty(1).each_with_index do |pkmn, i|
        ...
        # Pre-cache cry in RAM
        ::Audio.resolve_audio_file("cries/#{pkmn.species}", [".ogg", ".wav", ""], "Audio/SE/Cries") rescue nil
        ::Audio.resolve_audio_file("#{pkmn.species}", [".ogg", ".wav", ""], "Audio/SE/Cries") rescue nil
      end
    end
  end
