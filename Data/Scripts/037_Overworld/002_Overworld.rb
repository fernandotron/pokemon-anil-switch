...
EventHandlers.add(:on_step_taken, :grass_rustling,
  proc { |event|
    ...
    event.each_occupied_tile do |x, y|
      ...
      next if event.is_a?(Game_PokeEvent) && event.name == "pokemonTP"
      ...
      if $game_map.map_id==174
        spriteset&.addUserAnimation(24, x, y, true, 1)
      else
        spriteset&.addUserAnimation(Settings::GRASS_ANIMATION_ID, x, y, true, 1)
      end
    end
...
# Mostrar animación de pisar agua al pisar terrain tag 6 (id 6)
EventHandlers.add(:on_step_taken, :water_bubbles,
  proc { |event|
    next if !$scene.is_a?(Scene_Map)
    ...
    x = $game_player.x
    y = $game_player.y
    if currentTag.id_number == 6
      spriteset = $scene.spriteset($game_player.map_id)
      spriteset&.addUserAnimation(22, x, y, true, 3)
    end
  }
)
#=begin
EventHandlers.add(:on_step_taken, :water_bubbles_npcs,
  proc { |event|
    next if !$scene.is_a?(Scene_Map)
    next if event == $game_player
    ...
    # Obtenemos el terrain tag
    terrain_tag = $map_factory.getTerrainTagFromCoords(map_id, x, y, true)
    next if !terrain_tag || terrain_tag.id_number != 6
    # Mostramos la animación
    spriteset = $scene.spriteset(map_id)
    spriteset&.addUserAnimation(22, x, y, true, 0)
  }
)
#=end
...
def pbBattleOnStepTaken(repel_active)
  ...
  # Si es de tipo Cave salimos.
  return if encounter_type == :Cave && !pb_salvajes_visibles_en_ow? ## Añadido para las nubes de arena.
  return if encounter_type == :CaveClassic && !pb_salvajes_visibles_en_ow? ## Añadido para las nubes de arena.
  return if encounter_type == :Water && !pb_salvajes_visibles_en_ow?
  return if encounter_type == :WaterClassic && !pb_salvajes_visibles_en_ow?
...
def pbItemBall(item, quantity = 1, outfit_change = nil)
  ...
  #   if quantity > 1
  #     pbMessage(_INTL("¡Has obtenido {1} \\c[1]{2} {3}\\c[0]!", quantity, itemname, GameData::Move.get(move).name))
  #   else
  #     pbMessage(_INTL("¡Has obtenido \\c[1]{1} {2}\\c[0]!", itemname, GameData::Move.get(move).name))
  #   end
  # elsif quantity > 1
  #   pbMessage(_INTL("¡Has obtenido {1} \\c[1]{2}\\c[0]!", quantity, itemname))
  # elsif itemname.starts_with_vowel?
  #   pbMessage(_INTL("¡Has obtenido \\c[1]{1}\\c[0]!", itemname))
  # else
  #   pbMessage(_INTL("¡Has obtenido \\c[1]{1}\\c[0]!", itemname))
  # end
