if Settings::SHOW_ITEM_DESCRIPTIONS_ON_RECEIVE
  ...
  class PokemonItemFind_Scene
    ...
    def pbShow(item)
      ...
      if item_object.is_machine?
        ...
        move_data = GameData::Move.get(machine)
        name = _INTL("{1} {2}", name, move_data.name)
  ...
  class ItemLog
    ...
    def register(item)
      item_id = item.is_a?(Symbol) ? item : item.id
      if !@found_items.include?(item_id)
        @found_items.push(item_id)
        item_object = GameData::Item.try_get(item)
  ...
  def pbItemBall(item, quantity = 1, outfit_change = nil)
    result = pbItemBall_itemfind(item, quantity, outfit_change)
