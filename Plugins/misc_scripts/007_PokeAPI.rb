...
module PokeAPI
	...
	def parse_data(data, species)
		...
		# Extract sprite URLs
		sprites = data.fetch("sprites", {})
		sprite_urls = {}
		
		# Standard sprites
		sprite_urls["front_default"] = sprites["front_default"] if sprites["front_default"]
		sprite_urls["front_shiny"] = sprites["front_shiny"] if sprites["front_shiny"]
		sprite_urls["back_default"] = sprites["back_default"] if sprites["back_default"]
		sprite_urls["back_shiny"] = sprites["back_shiny"] if sprites["back_shiny"]
		
		# Animated sprites (from other object if available)
		other_sprites = sprites.fetch("other", {})
		showdown_sprites = other_sprites.fetch("showdown", {})
		
		if showdown_sprites["front_default"]
			sprite_urls["animated_front_default"] = showdown_sprites["front_default"]
		end
		if showdown_sprites["front_shiny"]
			sprite_urls["animated_front_shiny"] = showdown_sprites["front_shiny"]
		end
		if showdown_sprites["back_default"]
			sprite_urls["animated_back_default"] = showdown_sprites["back_default"]
		end
		if showdown_sprites["back_shiny"]
			sprite_urls["animated_back_shiny"] = showdown_sprites["back_shiny"]
		end
		
		data["sprite_urls"] = sprite_urls
		return data
	end
	...
	# Download animated sprite from PokeAPI directly without caching
	def download_animated_sprite(pokemon, back = false)
		return nil unless network_available?
		
		# Get Pokemon data from API first
		api_data = get_data(pokemon.species_data)

		return nil unless api_data && api_data["sprites"] && api_data["sprites"]["other"]

		sprite_urls = api_data["sprites"]["other"]

		if $PokemonSystem.streamer_mode_icon_type == 0
			sprite_urls = sprite_urls["showdown"]
			extension = 'gif'
		elsif $PokemonSystem.streamer_mode_icon_type == 1
			sprite_urls = sprite_urls["official_artwork"]
			extension = 'png'
		end

		sprite_key = if back
			pokemon.shiny? ? "back_shiny" : "back_default"
		else
			pokemon.shiny? ? "front_shiny" : "front_default"
		end

		sprite_key_female = sprite_key + (pokemon.female? ? "_female" : "")

		if sprite_urls.key?(sprite_key_female)
			sprite_key = sprite_key_female
		end

		sprite_url = sprite_urls[sprite_key]

		return nil unless sprite_url
		
		# Generate temporary filename
		temp_filename = "temp_sprite_#{pokemon.species}_#{pokemon.gender}_#{pokemon.form}_#{pokemon.shiny? ? '_shiny' : ''}.#{extension}"
		# Download sprite to temporary file
		begin
			pbDownloadToFile(sprite_url, temp_filename)

			if FileTest.exist?(temp_filename) && File.size(temp_filename) > 0
				return temp_filename
			else
				File.delete(temp_filename) if FileTest.exist?(temp_filename)
				return nil
			end
		rescue => e
			puts "Error downloading sprite: #{e.message}"
			File.delete(temp_filename) if FileTest.exist?(temp_filename)
			return nil
		end
	end
	
	# Non-blocking sprite getter for StreamOverlayUpdater
	def get_image(pokemon, callback = nil)
		return nil unless Overlay::USE_POKEAPI_FOR_SPRITES
		begin
			return download_animated_sprite(pokemon, false)
		rescue => e
			puts "Quick sync download failed: #{e.message}"
		end
		return nil
	end
end