      ...
      when 2
        begin 
          if bgGraphic.bitmap.nil?
            ...
            cr = i.colorRed ? (oldbg[3]&.red || 0.0) + ((i.colorRed - (oldbg[3].red || Color.new(0, 0, 0, 0).red)) * fraction) : oldbg[3]&.red || Color.new(0, 0, 0, 0).red
            cg = i.colorGreen ? (oldbg[3]&.green || 0.0) + ((i.colorGreen - (oldbg[3]&.green || Color.new(0, 0, 0, 0).green)) * fraction) : oldbg[3]&.green || Color.new(0, 0, 0, 0).green
            cb = i.colorBlue ? (oldbg[3]&.blue || 0.0) + ((i.colorBlue - (oldbg[3]&.blue || Color.new(0, 0, 0, 0).blue)) * fraction) : oldbg[3]&.blue || Color.new(0, 0, 0, 0).blue
            ca = i.colorAlpha ? (oldbg[3]&.alpha || 0.0) + ((i.colorAlpha - (oldbg[3]&.alpha || Color.new(0, 0, 0, 0).alpha)) * fraction) : oldbg[3]&.alpha || Color.new(0, 0, 0, 0).alpha
      ...
      when 4
        begin
          if foGraphic.bitmap.nil?
            ...
            cr = i.colorRed ? (oldfo[3]&.red || 0.0) + ((i.colorRed - (oldfo[3]&.red || Color.new(0, 0, 0, 0).red)) * fraction) : oldfo[3]&.red || Color.new(0, 0, 0, 0).red
            cg = i.colorGreen ? (oldfo[3]&.green || 0.0) + ((i.colorGreen - (oldfo[3]&.green || Color.new(0, 0, 0, 0).green)) * fraction) : oldfo[3]&.green || Color.new(0, 0, 0, 0).green
            cb = i.colorBlue ? (oldfo[3]&.blue || 0.0) + ((i.colorBlue - (oldfo[3]&.blue || Color.new(0, 0, 0, 0).blue)) * fraction) : oldfo[3]&.blue || Color.new(0, 0, 0, 0).blue
            ca = i.colorAlpha ? (oldfo[3]&.alpha || 0.0) + ((i.colorAlpha - (oldfo[3]&.alpha || Color.new(0, 0, 0, 0).alpha)) * fraction) : oldfo[3]&.alpha || Color.new(0, 0, 0, 0).alpha
...
def pbSpriteSetAnimFrame(sprite, frame, user = nil, target = nil, inEditor = false)
  ...
  sprite.blend_type = frame[AnimFrame::BLENDTYPE] || 0
  sprite.angle      = frame[AnimFrame::ANGLE] || 0
  sprite.mirror     = (frame[AnimFrame::MIRROR] && frame[AnimFrame::MIRROR] > 0)
  sprite.opacity    = frame[AnimFrame::OPACITY] || 255
  vis = frame[AnimFrame::VISIBLE]
  sprite.visible    = (vis.nil? || vis == true || vis != 0)
  ...
  sprite.zoom_x = (frame[AnimFrame::ZOOMX] || 100) / 100.0
  sprite.zoom_y = (frame[AnimFrame::ZOOMY] || 100) / 100.0
  sprite.color.set(
    frame[AnimFrame::COLORRED] || 0,
    frame[AnimFrame::COLORGREEN] || 0,
    frame[AnimFrame::COLORBLUE] || 0,
  ...
  sprite.x  = frame[AnimFrame::X] || 0
  sprite.y  = frame[AnimFrame::Y] || 0
...
class PBAnimationPlayerX
  ...
  def start
    ...
    if !@animbitmap || @animbitmap.disposed?
      if @animation && @animation.graphic && @animation.graphic != ""
        @animbitmap = (pbGetAnimation(@animation.graphic, @animation.hue) rescue nil)
        (2...MAX_SPRITES).each do |i|
  ...
  def update
    return if @frame < 0
    if (!@animbitmap || @animbitmap.disposed?) && @animation && @animation.graphic && @animation.graphic != ""
      @animbitmap = (pbGetAnimation(@animation.graphic, @animation.hue) rescue nil)
      (2...MAX_SPRITES).each do |i|
        @animsprites[i].bitmap = @animbitmap if @animsprites[i]
      end
    ...
    @foGraphic.update
    @foColor.update
    # Update all the sprites to depict the animation's frame
    thisframe = @animation[@frame]
    if thisframe
      # Make only particle cel sprites (indices 2+) invisible
      (2...MAX_SPRITES).each { |i| @animsprites[i].visible = false if @animsprites[i] }
      ...
      thisframe.length.times do |i|
        ...
        when 1   # Focused on target
          sprite.x = (cel[AnimFrame::X] || 0) + @targetOrig[0] - Battle::Scene::FOCUSTARGET_X
          sprite.y = (cel[AnimFrame::Y] || 0) + @targetOrig[1] - Battle::Scene::FOCUSTARGET_Y
        when 2   # Focused on user
          sprite.x = (cel[AnimFrame::X] || 0) + @userOrig[0] - Battle::Scene::FOCUSUSER_X
          sprite.y = (cel[AnimFrame::Y] || 0) + @userOrig[1] - Battle::Scene::FOCUSUSER_Y
        when 3   # Focused on user and target
          if @srcLine && @dstLine
            ...
            if isReversed(@srcLine[0], @srcLine[2], @dstLine[0], @dstLine[2]) &&
               cel[AnimFrame::PATTERN] && cel[AnimFrame::PATTERN] >= 0
              # Reverse direction
