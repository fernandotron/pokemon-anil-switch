...
module VisibleEncounterSettings
  AUTO_SPAWN_SPEED = 45 # Ajustado para aparición natural en reposo (cada 0.75s a 60 FPS)
...
EventHandlers.add(:on_frame_update, :automatic_spawning,
  proc {
    ...
    $framecounter = 0
    pbSpawnOnStepTaken(repel_active) if !pbBattleOrSpawnOnStepTaken(repel_active) && $PokemonSystem.salvajes_visibles_en_ow == 0
  }
)
