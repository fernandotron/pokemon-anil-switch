...
class Swdfm_Exp_Screen
  def initialize(values)
    ...
    @total_frames  = (@values.max.to_f / speed).floor
    @total_frames  = @total_frames.clamp((40 * FASTEST_TIME).to_i, (40 * SLOWEST_TIME).to_i)
    @sprites    = {}
    @width      = Graphics.width
    @height     = Graphics.height
    @viewport   = Viewport.new(0, 0, @width, @height)
    @viewport.z = 120_000
    $active_exp_panel_viewport = @viewport
    @do_break   = false
    before_run
    run
  end
  ...
  def update_bars
    ...
    for i in 0...$player.party.size
      next if !$player.party[i]
      next if $player.party[i].level >= Settings::MAXIMUM_LEVEL
      next if !@values[i] || @values[i] <= 0
      mock_exp  = $player.party[i].exp + (@values[i] * @elapsed / @total_frames).floor
      lvl, perc = $player.party[i].exp_fraction_for_panel(mock_exp)
      if lvl && @levels[i] && lvl > @levels[i]
        @levels[i] = lvl
        redraw_level(i)
        @sprites["bar_1_#{i}"]&.bitmap&.clear
      end
      if @elapsed == (ANNOUCE_TIME * 40).to_i || @elapsed == @total_frames
        dispose_if_there("exp_#{i}")
      end
      next unless @sprites["bar_1_#{i}"] && !@sprites["bar_1_#{i}"].disposed?
      bmp = @sprites["bar_1_#{i}"].bitmap
      next unless bmp && !bmp.disposed?
      p_clamped = [[perc.to_f, 0.0].max, 100.0].min
      fill_w = (bmp.width * p_clamped / 100.0).floor
      bmp = Swdfm_Bitmap.colour(bmp, EXP_EXP_COLOUR, 0, 0, fill_w, bmp.height)
  ...
  def draw_party
    ...
    b_h = EXP_BAR_HEIGHT
    party_size = [[$player.party.length, 1].max, 6].min
    c_o = CO_ORDINATES[party_size - 1]
    e   = EXP_BAR_EDGE_SIZE
    @places = []
    for i in 0...$player.party.length
      break if i >= c_o.length
      x_c, y_c = c_o[i]
      x        = x_c * @s_width
      y        = y_c * @s_height
      x       -= (b_w / 2)
  ...
  def redraw_level(i)
    return unless @sprites["level_#{i}"] && !@sprites["level_#{i}"].disposed?
    @sprites["level_#{i}"].bitmap&.clear
    pbSEPlay("Pkmn exp full") rescue nil
    hash = {
      :X       => 48,
      :Align   => 2,
      :Outline => true
