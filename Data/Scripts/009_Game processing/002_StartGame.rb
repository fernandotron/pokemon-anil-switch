...
module Game
  ...
  def self.initialize
    log_compat("[Game.initialize] 1. Creando Game_Temp y Game_System...") rescue nil
    $game_temp          = Game_Temp.new
    $game_system        = Game_System.new
    log_compat("[Game.initialize] 2. Cargando Animations, Tilesets, CommonEvents, System...") rescue nil
    ...
    log_compat("[Game.initialize] 5. Inicialización completada con éxito.") rescue nil
  end
  ...
  def self.load(save_data)
    ...
    self.load_map
    (pbAutoplayOnSave rescue nil)
    $game_map.update rescue nil
    $PokemonMap.updateMap rescue nil
    $scene = Scene_Map.new
  end
