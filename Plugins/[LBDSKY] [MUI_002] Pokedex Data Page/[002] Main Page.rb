...
class PokemonPokedexInfo_Scene 
  ...
  def pbDataPageMenu
    ...
    @api_data = PokeAPI.get_data(species_data)
    loop do
      Graphics.update
      Input.update
      pbUpdate
      ...
      #-------------------------------------------------------------------------
      elsif Input.trigger?(Input::SPECIAL) && @api_data 
        pbPlayDecisionSE
        @api_data = PokeAPI.get_data(species_data) if @api_data["species"] != species || @api_data["form"] != @form
        if @cursor == :stats
          stat_diffs = []
          api_total = 0
          species_data = GameData::Species.get_species_form(@species, @form)
  ...
  def pbGenerateDataLists(species)
    ...
    family_to_insert = []
    blacklisted = [:PICHU_2, :FLOETTE_5].include?(species.id) ||
  ...
  def drawPageData
    ...
    if special_form
      ...
      #-------------------------------------------------------------------------
        ...
        form = 2 if species_data.id == :DARMANITAN_3
