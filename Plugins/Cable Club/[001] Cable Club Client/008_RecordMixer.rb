...
module CableClub
  def self.do_mix_records(msgwindow,connection)
    RecordMixer.each do |sym|
      ...
      yield _INTL("Preparando {1} Datos",record_name) if block_given?
    ...
    RecordMixer.each do |sym|
      ...
      ret = false
      loop do
        yield _INTL("Recibiento {1} Datos",record_name) if block_given?
        Graphics.update
        Input.update
    ...
    RecordMixer.each do |sym|
      record_name = RecordMixer.record_name(sym)
      yield _INTL("Procesando {1} Datos",record_name) if block_given?
