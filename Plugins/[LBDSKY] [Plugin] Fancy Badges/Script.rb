...
def analyzeBitmap3D(bitmap, reverse = false)
  ...
  for y in 0...bitmap.height
    ...
    for x in 0...bitmap.width
      ...
      x0 = x
      color0.set(color.red, color.green, color.blue, color.alpha)
    end
...
def renderBadgeAnimation(badge_number = 0)
  ...
  begin
    ...
    for frame in 0...224
      # viewport flash
      viewport.color.alpha = [0, viewport.color.alpha - 25].max if viewport.color.alpha > 0
      ...
      (0...height).step(2) do |y|
        ...
        if !points[y].nil?
          for l in 0...points[y].length
            ...
            w2 = w2.ceil
            side_temp.stretch_blt(Rect.new(x2,y,w2,2),side,Rect.new(0,y2,width_side,2))
          end
          y2 += 2
        end
      end
      (0...width).step(2) do |x|
        ...
        y = y.floor
        badge_s.bitmap.stretch_blt(Rect.new(x,y+offset,2,h),side_temp,Rect.new(x,0,2,side_temp.height))
      end
      (0...height).step(2) do |y|
        w = width*zoom_x
        z2 = 1.0 - zoom_x
        ...
        temp.stretch_blt(Rect.new(x,y+offset,w,2),isBack ? back : front,Rect.new(0,y,width,2))
      end
      (0...width).step(2) do |x|
        h = temp.height*ratio1[x]
        y = (temp.height - h)*0.5
        y = y.floor
        badge_f.bitmap.stretch_blt(Rect.new(x,y,2,h),temp,Rect.new(x,0,2,temp.height))
      end
      ...
      t1 = tone*(1 - zoom_x)
      badge_f.tone.set(t1,t1,t1)
      t2 = tone*(zoom_x)
      badge_s.tone.set(t2,t2,t2)
      # animation for rays
      for j in 0...8
    ...
    [badge_s, badge_f, temp, side_temp, front, back, side].each do |s|
      begin
        s.bitmap.dispose if s && s.respond_to?(:bitmap) && s.bitmap && !s.bitmap.disposed?
        s.dispose if s && !s.disposed?
      rescue
        nil
      end
    end
    begin; fp["bg"].bitmap.dispose if fp && fp["bg"] && fp["bg"].bitmap && !fp["bg"].bitmap.disposed? && fp["bg"].bitmap != screen; rescue; nil; end
    begin; fp["text"].bitmap.dispose if fp && fp["text"] && fp["text"].bitmap && !fp["text"].bitmap.disposed?; rescue; nil; end
    begin; screen.dispose if screen && !screen.disposed?; rescue; nil; end
    begin; pbDisposeSpriteHash(fp) if fp; rescue; nil; end
    begin; viewport.dispose if viewport && !viewport.disposed?; rescue; nil; end
  end
end
#-------------------------------------------------------------------------------
