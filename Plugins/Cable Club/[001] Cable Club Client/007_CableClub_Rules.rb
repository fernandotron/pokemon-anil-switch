...
class PokemonRuleSet
  def hasRegistrableTeam?(list)
    ...
    (1..self.maxTeamLength).each do |x|
...
class PokemonOnlineRules
  ...
  attr_reader :random_battle
  attr_reader :random_config
  ...
  def random_battle?; return @random_battle; end

  def setRandomBattle
    @random_battle = true
  end

  def unsetRandomBattle
    @random_battle = false
  end

  def set_random_battle_config(config)
  ...
  def adjustLevels(party1,party2)
    if @levelAdjustment && @levelAdjustment.type==LevelAdjustment::BOTH_TEAMS
      ret = @levelAdjustment.adjustLevels(party1,party2)
      return ret
    else
      return nil
    end
