...
class Pokemon
  ...
  # Cached super shiny hue value to maintain consistent colors
  attr_accessor :cached_super_shiny_hue
  ...
  MAX_NAME_SIZE = 12 # Por defecto 10
  ...
  def level=(value)
    if value < 1 || value > GameData::GrowthRate.max_level
      raise ArgumentError.new(_INTL("El número del nivel ({1}) es inválido.", value))
    end
  ...
  def status=(value)
    ...
    if !new_status
      raise ArgumentError, _INTL("Intento de definir {1} como estado del Pokémon", value.class.name)
    end
  ...
  def can_relearn_move?
    ...
    if Settings::SHOW_MTS_MOS_IN_MOVE_RELEARNER
      tm_moves = pbGetTMMoves(self)
      return true if tm_moves && !tm_moves.empty?
    end
    return false
  end
  ...
  def timeEggHatched
    return (obtain_method == 1) && @timeEggHatched ? Time.at(@timeEggHatched) : nil
  end
  ...
  def calcHP(base, level, iv, ev)
    ...
    iv = ev = 0 if Settings::DISABLE_IVS_AND_EVS || ($game_switches && $game_switches[MODO_SIN_GRINDEO])
  ...
  def initialize(species, level, owner = $player, withMoves = true, recheck_form = true)
    ...
    raise "No species data loaded" if !species_data
