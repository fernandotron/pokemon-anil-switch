...
def pbPreloadAudioSE(name)
  ...
  if defined?(Audio) && Audio.respond_to?(:se_play)
    Audio.se_play(name, 0, 100) rescue nil
  end
end
...
def pbPreloadMapEncountersAudio(map_id = nil)
  ...
  enc_data = $PokemonEncounters.setup(map_id) rescue nil
  return unless enc_data
  
  species_list = []
  if $PokemonEncounters.respond_to?(:encounter_tables)
    tables = $PokemonEncounters.encounter_tables rescue nil
    if tables.is_a?(Hash)
      tables.each_value do |enc_array|
        next unless enc_array.is_a?(Array)
        enc_array.each do |enc|
          if enc.is_a?(Array) && enc[1]
            species_list << enc[1]
          end
        end
      end
  ...
  species_list.uniq.each do |sp|
    cry_file = GameData::Species.cry_filename(sp) rescue nil
    if cry_file && !cry_file.empty?
      pbPreloadAudioSE(cry_file)
    end
    if defined?($GRAPHICS_LOOKUP_TABLE) && $GRAPHICS_LOOKUP_TABLE
      ["Followers/#{sp}", "#{sp}"].each do |cand|
        pbResolveBitmap("Graphics/Characters/" + cand) rescue nil
      end
    end
  end
rescue Exception => e
...
def pbWarmupCoreAssets!
  return if $CORE_ASSETS_WARMED_UP
  $CORE_ASSETS_WARMED_UP = true
  ...
  common_ses.each do |se|
    begin
      Audio.se_play("Audio/SE/" + se, 0, 100)
    rescue Exception => e
    end
  end
  Audio.se_stop rescue nil
  
  if defined?($data_animations) && $data_animations
    [29, 30].each do |anim_id|
      anim = $data_animations[anim_id] rescue nil
      if anim && anim.animation_name && !anim.animation_name.empty?
        RPG::Cache.load_bitmap("Graphics/Animations/", anim.animation_name) rescue nil
      end
    end
  end
  
  DP_PauseMenu.precache_assets rescue nil if defined?(DP_PauseMenu)
end

EventHandlers.add(:on_map_transfer, :preload_map_encounters_audio,
  proc { |_old_map_id|
    pbPreloadMapEncountersAudio rescue nil
    pbWarmupCoreAssets! rescue nil
  }
)

EventHandlers.add(:on_game_load, :warmup_audio_on_load,
  proc {
    pbWarmupCoreAssets! rescue nil
  }
)
