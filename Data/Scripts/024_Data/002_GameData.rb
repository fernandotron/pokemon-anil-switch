module GameData
  ...
  module ClassMethods
    ...
    def exists?(other)
      self.load if self::DATA.empty? && self.const_defined?(:DATA_FILENAME)
    ...
    def get(other)
      ...
      if !self::DATA.has_key?(other)
        if self.to_s.include?("TrainerType")
          other = :POKEMONTRAINER_RojoNeutro if [:POKEMONTRAINER_Red, :POKEMONTRAINER_Leaf, :POKEMONTRAINER_RED, :POKEMONTRAINER_LEAF].include?(other)
          return self::DATA[other] if self::DATA.has_key?(other)
        end
        return self::DATA.values.first if !self::DATA.empty?
        raise "Unknown ID #{other}."
      end
