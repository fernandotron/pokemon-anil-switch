...
class Window_AdvancedTextPokemon < SpriteWindow_Base
  ...
  def refresh
    ...
    if @needclear
      self.contents.font = @oldfont if @oldfont rescue nil
      self.contents.clear
      @needclear = false
    end
    if @nodraw
    ...
    self.contents.font = @oldfont if !self.letterbyletter && @oldfont rescue nil
...
class Window_AdvancedCommandPokemon < Window_DrawableCommand
  ...
  def initialize(commands, width = nil)
    ...
    self.active = true
    colors = getDefaultTextColors(self.windowskin)
    self.baseColor = MessageConfig::LIGHT_TEXT_MAIN_COLOR #colors[0]
    self.shadowColor = MessageConfig::LIGHT_TEXT_SHADOW_COLOR #colors[1]
    refresh
    @starting = false
  end
