...
class PokemonSystem
  ...
  attr_accessor :show_pokemon_on_change
  ...
  def initialize
    ...
    @show_pokemon_on_change = 0 # Show Pokémon on change (0=show, 1=hide)
  ...
  def vsync_initial_value?
    return @vsync if @vsync
    return 1 if !FileTest.exist?("mkxp.json") || $joiplay
...
class Window_PokemonOption < Window_DrawableCommand
  ...
  SEL_NAME_BASE_COLOR    = Color.new(248, 176, 80)
  SEL_NAME_SHADOW_COLOR  = Color.new(142, 90, 0)
  SEL_VALUE_BASE_COLOR   = Color.new(248, 136, 128)
  SEL_VALUE_SHADOW_COLOR = Color.new(139, 25, 12) 
  ...
  def drawItem(index, _count, rect)
    ...
    # Draw option's name
    optionname = (index == @options.length) ? (@is_sub_menu ? _INTL("Atrás") : _INTL("Cerrar")) : @options[index].name
...
class PokemonOption_Scene
  ...
  def pbStartScene(in_load_screen = false, options_menu = :options_menu, is_sub_menu = false)
    ...
    pbBottomLeftLines(@sprites["textbox"], 3)
    pbSetSystemFont(@sprites["textbox"].contents)
    @sprites["option"] = Window_PokemonOption.new(
      @options, 0, @sprites["title"].y + @sprites["title"].height - 16, Graphics.width,
      Graphics.height - (@sprites["title"].y + @sprites["title"].height - 16) - @sprites["textbox"].height, @is_sub_menu
    )
    @sprites["option"].viewport = @viewport
    @sprites["option"].visible  = true
    # Warm up audio & font caches
    pbResolveAudioSE("GUI sel cursor") rescue nil
    pbResolveAudioSE("GUI menu close") rescue nil
    pbResolveAudioSE("GUI menu open") rescue nil
  ...
  def pbChangeSelection
    ...
    if hash
      ...
      description = @is_sub_menu ? _INTL("Vuelve al menú anterior.") : _INTL("Cierra el menú de opciones.")
    end
  ...
  def pbOptions
    ...
    # Borramos los salvajes visibles si se han desactivado.
    delete_all_wild_pkmn_spawned() if !$PokemonSystem.salvajes_visibles_en_ow?
  end
...
class PokemonOptionScreen
  ...
  def pbStartScreen(in_load_screen = false, options_menu = :options_menu, is_sub_menu = false)
    @scene.pbStartScene(in_load_screen, options_menu, is_sub_menu)
    @scene.pbOptions
    @scene.pbEndScene
...
MenuHandlers.add(:options_menu, :show_change_pokemon, {
  "name"        => _INTL("Mostrar sigu. Pkmn."),
  "order"       => 51,
  "type"        => EnumOption,
  "parameters"  => [_INTL("Mostrar"), _INTL("Ocultar")],
  "description" => _INTL("Elige si quieres que se te diga qué Pokémon saldrá al derrotar un Pokémon rival y preguntarte si cambiarlo por otro."),
  "get_proc"    => proc { next $PokemonSystem.show_pokemon_on_change },
  "set_proc"    => proc { |value, _scene| $PokemonSystem.show_pokemon_on_change = value }
})
