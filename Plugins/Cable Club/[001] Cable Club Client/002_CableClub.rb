...
class Player < Trainer
  ...
  def online_battles_wins
    online_stats[:battles_wins]
  end

  def online_battles_lost
    online_stats[:battles_lost]
  end

  def online_trade_count
    online_stats[:trade_count]
  end

  def register_online_win
    online_stats[:battles_wins] += 1
  end

  def register_online_loss
    online_stats[:battles_lost] += 1
  end

  def register_online_trade
    online_stats[:trade_count] += 1
  end
end

module CableClub
  @@fainted_before_battle = []

  ACTIVITY_OPTIONS = {:battle => _INTL("Combate"),
                      :trade => _INTL("Intercambio"),
                      :record_mix => _INTL("Mezclar registros")}
  def self.pokemon_order(client_id)
    case client_id % 2
    when 0
      [0, 2, 1, 3, 4, 5]  # Jugador 0 ve: sus Pokémon, luego los del rival
    when 1
      [1, 3, 0, 2, 5, 4]  # Jugador 1 ve: sus Pokémon, luego los del rival
    else
      raise "Unknown client_id: #{client_id}"
    end
  end

  def self.pokemon_order_priority(_client_id)
    [0, 1, 2, 3, 4, 5]
  end
  ...
  def self.pokemon_target_order(client_id)
    case client_id
    when 0
      [1, 0, 3, 2, 5, 4]   # Cliente 0 ve: 0 y 2 son suyos, 1 y 3 del rival
    when 1
      [1, 0, 3, 2, 5, 4]   # Cliente 1 ve: 1 y 3 suyos, 0 y 2 del rival (se reordenan igual)
    else
      raise "Unknown client_id: #{client_id}"
    end
  end

  def self.record_fainted_pokemon(party)
    @@fainted_before_battle = []
    party.each_with_index do |pkmn, i|
      @@fainted_before_battle << i if pkmn && pkmn.fainted?
    end
  end

  def self.heal_battle_fainted_pokemon(party)
    return if @@fainted_before_battle.nil?
    
    party.each_with_index do |pkmn, i|
      next if !pkmn || @@fainted_before_battle.include?(i)
      # Solo cura si el Pokémon se debilitó durante el combate
      # (no estaba debilitado antes pero ahora sí)
      if pkmn.fainted?
        pkmn.curar_equipo_debilitado_debug if pkmn.respond_to?(:curar_equipo_debilitado_debug)
      end
    end
  ...
  def self.do_battle(connection, client_id, seed, battle_rules, player_party, partner, partner_party)
    if !battle_rules.random_battle? && ChallengeModes.on?(:PERMAFAINT)
      record_fainted_pokemon($player.party)
    end
    ...
    setBattleRule("battlebgm", "Online.ogg")
    ...
    # Saltamos al mapa del combate.
    self.transfer_to_battle_room if !$game_switches[SWITCH_ONLINE_COMBATE] # Solo vamos a la sala online si aún no estamos ahí.

    pbBattleAnimation(nil, (battle.singleBattle?) ? 1 : 3, [partner]) {
      pbSceneStandby {
        begin
          ...
          # NUEVA FUNCIONALIDAD: Curar Pokémon que se debilitaron solo durante el combate
          # Solo para batallas con Pokémon del jugador (no de préstamo) y en modo Nuzlocke con muerte permanente
          if !battle_rules.random_battle? && ChallengeModes.on?(:PERMAFAINT)
            heal_battle_fainted_pokemon($player.party)
          end
        end
      }
    }
    ...
    case outcome
    when 1
      $player.register_online_win
    when 2, 3 
      $player.register_online_loss
    end
  end
  ...
  def self.do_trade(index, you, your_pkmn)
    ...
    self.transfer_to_trade_room if !$game_switches[SWITCH_ONLINE_INTERCAMBIO] 
  ...
  def self.choose_pokemon
    ...
    pbFadeOutIn(99999) {
      ...
      screen.pbStartScene(_INTL("Elige un Pokémon para intercambiar."), false)
      loop do
        pkmnid = screen.pbChoosePokemon
        break if pkmnid < 0
        pkmn = $player.party[pkmnid]
        next if !pkmn
        commands = []
        cmdSelect = cmdSummary = -1
        commands[cmdSelect = commands.length] = _INTL("Seleccionar")
        commands[cmdSummary = commands.length] = _INTL("Ver datos")
  ...
  def self.write_pkmn(writer, pkmn)
    ...
    writer.nil_or(:bool,pkmn.super_shiny?) # super shiny
    ...
    if pkmn.hasMegaForm?
      pkmn_aux = pkmn.dup
      pkmn_aux.makeMega
      pkmn.mega_ability = pkmn_aux.ability_id if pkmn_aux.ability_id
      writer.nil_or(:sym, pkmn.mega_ability)
      pkmn_aux.makeUnmega
    else
      writer.nil_or(:sym, nil)
    end
  ...
  def self.parse_pkmn(record)
    ...
    pkmn.super_shiny = record.nil_or(:bool) # super shiny
    ability = record.nil_or(:sym)
    pkmn.ability = ability
    ability_index = record.nil_or(:int)
    # pkmn.ability_index = nil #ability_index
    pkmn.forced_ability = pkmn.ability if defined?(pkmn.forced_ability) && defined?(RandomizedChallenge) && RandomizedChallenge.random_abilities?
    ...
    pkmn.mega_ability = record.nil_or(:sym)
    pkmn.calc_stats
    RandomizedChallenge.resume_random_species if random && defined?(RandomizedChallenge)
    return pkmn
  end
  ...
  def self.parse_battle_rule(record)
    ...
    if record.respond_to?(:bool)
      rule.setRandomBattle if record.bool
      random_config = {
        allow_mega:             record.bool,
        allow_mega_item:        record.bool,
        allow_singulars:        record.bool,
        allow_major_legendaries:record.bool,
        allow_minor_legendaries:record.bool,
        allow_ultra_beasts:     record.bool,
        allow_paradoxes:        record.bool
      }
      rule.set_random_battle_config(random_config)
    end
  ...
  def self.write_battle_rule(writer,battle_rule)  
    ...
    writer.bool(battle_rule[2].random_battle?)
    # Ecribimos también la config del random
    # Escribimos la configuración del combate aleatorio
    random_config = battle_rule[2].random_config || {}
    writer.bool(random_config[:allow_mega])
    writer.bool(random_config[:allow_mega_item])
    writer.bool(random_config[:allow_singulars])
    writer.bool(random_config[:allow_major_legendaries])
    writer.bool(random_config[:allow_minor_legendaries])
    writer.bool(random_config[:allow_ultra_beasts])
    writer.bool(random_config[:allow_paradoxes])
  end
  
  def self.get_server_info
    ret = [HOST,PORT]
    if FileTest.exist?(File.join("Data","serverinfo.ini"))
      File.foreach(File.join("Data","serverinfo.ini")) do |line|
        case line
  ...
  def self.transfer_to_battle_room
    return false if !$player.connecting_online?

    # Seteamos el sprite del rival en base a su trainertpe
    case $partner_online[1]
    when :POKEMONTRAINER_RojoNeutro
      $game_variables[77] = 0
    when :POKEMONTRAINER_RojoArio
      $game_variables[77] = 1
    when :POKEMONTRAINER_RojoNegro
      $game_variables[77] = 2
    when :POKEMONTRAINER_HojaNeutra
      $game_variables[77] = 3
    when :POKEMONTRAINER_HojaAria
      $game_variables[77] = 4
    when :POKEMONTRAINER_HojaNegra
      $game_variables[77] = 5
    when :POKEMONTRAINER_Yellow
      $game_variables[77] = 6
    else
      $game_variables[77] = 0
    end

    map_id = 218  # cambia esto por el ID real del mapa del combate
    x = 50
    y = 25
    pbFadeOutIn(99999){
    ...
    pbWait(1)
    # Hacer que el jugador dé tres pasos hacia adelante.
    3.times {$game_player.move_up
    ...
    pbWait(2)
    pbMessage(_INTL("¡Comienza el combate contra {1}! ¡Adelante!",$partner_online[0]))
    pbWait(1)
    # pbCameraReset

    # Activamos el SWITCH de estar en combate.
    $game_switches[SWITCH_ONLINE_COMBATE] = true
    $game_switches[SWITCH_ONLINE_INTERCAMBIO] = false
  end




  def self.transfer_to_trade_room
    return false if !$player.connecting_online?

    # Seteamos el sprite del rival en base a su trainertpe
    echoln $partner_online
    case $partner_online[1]
    when :POKEMONTRAINER_RojoNeutro
      $game_variables[77] = 0
    when :POKEMONTRAINER_RojoArio
    ...
    pbWait(1)
    # Hacer que el jugador dé cuatro pasos hacia adelante.
    4.times {$game_player.move_up
      while $game_player.moving?
        Graphics.update
        Input.update
        pbUpdateSceneMap
    ...
    pbWait(0.6)
    # Encendemos la pantalla del PC.
    $game_switches[ENCENDER_PC_ONLINE] = true
    Graphics.update
    Input.update
    pbUpdateSceneMap
    ...
    pbWait(0.6)
    pbMessage(_INTL("¡Comienza el intercambio con {1}!", $partner_online[0]))
    pbWait(1)

    # Activamos el SWITCH de estar en combate.
    $game_switches[SWITCH_ONLINE_INTERCAMBIO] = true
    $game_switches[SWITCH_ONLINE_COMBATE] = false
  end
