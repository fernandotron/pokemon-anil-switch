...
class Battle::Scene
  ...
  # Text colors
  MESSAGE_BASE_COLOR   = Color.new(255,255,255) # Color.new(80, 80, 88)
  MESSAGE_SHADOW_COLOR = Color.new(75,75,75)    # Color.new(160, 160, 168)
  # The number of party balls to show in each side's lineup.
  NUM_BALLS            = max_party_size #Settings::MAX_PARTY_SIZE
  # Centre bottom of the player's side base graphic
  PLAYER_BASE_X        = 128
  PLAYER_BASE_Y        = Settings::SCREEN_HEIGHT - 80 + 100
  # Centre middle of the foe's side base graphic
  FOE_BASE_X           = Settings::SCREEN_WIDTH - 128
  FOE_BASE_Y           = (Settings::SCREEN_HEIGHT * 3 / 4) - 112 - 18
  ...
  def self.pbBattlerPosition(index, sideSize = 1)
    ...
    if (index & 1) == 0
      ret = [PLAYER_BASE_X, PLAYER_BASE_Y-100]
    else
      ret = [FOE_BASE_X, FOE_BASE_Y+18]
    end
  ...
  def pbShowWindow(windowType)
    ...
    @sprites["messageBox"]&.visible    = (windowType == MESSAGE_BOX) if @sprites["messageBox"] && !@sprites["messageBox"].disposed?
    @sprites["messageWindow"]&.visible = (windowType == MESSAGE_BOX) if @sprites["messageWindow"] && !@sprites["messageWindow"].disposed?
    @sprites["commandWindow"]&.visible = (windowType == COMMAND_BOX) if @sprites["commandWindow"] && !@sprites["commandWindow"].disposed?
    @sprites["fightWindow"]&.visible   = (windowType == FIGHT_BOX) if @sprites["fightWindow"] && !@sprites["fightWindow"].disposed?
    @sprites["targetWindow"]&.visible  = (windowType == TARGET_BOX) if @sprites["targetWindow"] && !@sprites["targetWindow"].disposed?
  end
  ...
  def pbChangePokemon(idxBattler, pkmn)
    ...
    # Update data box
    @sprites["dataBox_#{idxBattler}"]&.battler = @battle.battlers[idxBattler]
    @sprites["dataBox_#{idxBattler}"]&.pbRefreshOne(idxBattler)
  end
