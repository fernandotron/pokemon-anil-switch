...
class Game_FollowerFactory
  ...
  def add_follower(event, name = nil, common_event_id = nil)
    ...
    if name == "FollowingPkmn" || name == "FollowerPkmn"
      followers.delete_if { |data| data.name == "FollowingPkmn" || data.name == "FollowerPkmn" }
      @events.delete_if { |ev| ev.is_a?(Game_FollowingPkmn) } rescue nil
    elsif followers.any? { |data| data.original_map_id == $game_map.map_id && data.event_id == (event.respond_to?(:id) ? event.id : 0) }
      return   # Event is already dependent
    end
    ev_id = event.respond_to?(:id) ? event.id : 0
    ev_name = event.respond_to?(:name) ? event.name.to_s : ""
    c_name = event.character_name ? event.character_name.to_s : ""
    c_hue = event.character_hue || 0
    eventData = FollowerData.new($game_map.map_id, ev_id, ev_name,
  ...
  def followers_need_move?
    return false if !@events || @events.empty?
    lead = $game_player
    return false if !lead
    @events.any? { |ev| ev && !ev.moving? && !ev.jumping? && (ev.x != lead.x || ev.y != lead.y) }
  end
  ...
  def update
    ...
    followers.each_with_index do |follower, i|
      event = @events[i]
      next if !@events[i]
      if follower.respond_to?(:following_pkmn?) && follower.following_pkmn?
        follower.invisible_after_transfer = false
        if (FollowingPkmn.active? rescue true) && ($PokemonGlobal&.follower_toggled != false)
      ...
      elsif follower.invisible_after_transfer && player_moving
        follower.invisible_after_transfer = false
        event.turn_towards_leader($game_player)
      end
      event.move_speed  = leader.move_speed
      event.transparent = !follower.visible? unless (follower.respond_to?(:following_pkmn?) && follower.following_pkmn?)
...
class FollowerSprites
  ...
  def refresh
    ...
    @sprites.clear
    return if !$game_temp || !$game_temp.followers
    $game_temp.followers.each_follower do |event, follower|
      @sprites.push(Sprite_Character.new(@viewport, event)) rescue nil
    end
  end

  def update
    cur_update = $game_temp&.followers&.last_update rescue nil
    events_count = ($game_temp&.followers&.events&.length rescue 0)
    if cur_update != @last_update || @sprites.length != events_count
      refresh
      @last_update = cur_update
    end
    @sprites.each { |sprite| sprite.update rescue nil }
  end
end
