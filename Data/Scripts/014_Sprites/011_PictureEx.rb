...
def setPictureSprite(sprite, picture, iconSprite = false)
  ...
  picture.frameUpdates.each do |type|
    case type
    when Processes::XY, Processes::DELTA_XY
      sprite.send(:x=, picture.x.round) rescue (sprite.x = picture.x.round rescue nil)
      sprite.send(:y=, picture.y.round) rescue (sprite.y = picture.y.round rescue nil)
    when Processes::Z
      sprite.send(:z=, picture.z) rescue (sprite.z = picture.z rescue nil)
    when Processes::ZOOM
      sprite.send(:zoom_x=, picture.zoom_x / 100.0) rescue (sprite.zoom_x = picture.zoom_x / 100.0 rescue nil)
      sprite.send(:zoom_y=, picture.zoom_y / 100.0) rescue (sprite.zoom_y = picture.zoom_y / 100.0 rescue nil)
    when Processes::ANGLE
      sprite.angle = picture.angle
    when Processes::TONE
      if sprite.tone && picture.tone
        sprite.tone.set(picture.tone.red, picture.tone.green, picture.tone.blue, picture.tone.gray) rescue (sprite.tone = picture.tone rescue nil)
      end
    when Processes::COLOR
      if sprite.color && picture.color
        sprite.color.set(picture.color.red, picture.color.green, picture.color.blue, picture.color.alpha) rescue (sprite.color = picture.color rescue nil)
      end
    when Processes::HUE
