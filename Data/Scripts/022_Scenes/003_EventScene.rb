...
class PictureSprite < Sprite
  ...
  def update
    ...
    if @picture
      self.x = @picture.x.round rescue 0
      self.y = @picture.y.round rescue 0
      self.z = @picture.z || 100
      self.zoom_x = ((@picture.zoom_x || 100.0) / 100.0) rescue 1.0
      self.zoom_y = ((@picture.zoom_y || 100.0) / 100.0) rescue 1.0
      self.angle = @picture.angle rescue 0
      self.blend_type = @picture.blend_type rescue 0
      self.opacity = @picture.opacity rescue 255
      self.visible = @picture.visible && (@picture.name != "" || @customBitmap != nil)
    end
