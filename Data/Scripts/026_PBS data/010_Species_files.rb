module GameData
  class Species
    $GRAPHIC_FILE_CACHE ||= {}
    $EGG_GRAPHIC_FILE_CACHE ||= {}
    $CRY_FILE_CACHE ||= {}

    def self.check_graphic_file(path, species, form = 0, gender = 0, shiny = false, shadow = false, subfolder = "")
      cache_key = [path, species, form, gender, shiny, shadow, subfolder]
      return $GRAPHIC_FILE_CACHE[cache_key] if $GRAPHIC_FILE_CACHE.has_key?(cache_key)
      ...
      (2**factors.length).times do |i|
        ...
        if ret
          $GRAPHIC_FILE_CACHE[cache_key] = ret
          return ret
        end
      end
      $GRAPHIC_FILE_CACHE[cache_key] = nil
      return nil
    end

    def self.check_egg_graphic_file(path, species, form, suffix = "")
      cache_key = [path, species, form, suffix]
      return $EGG_GRAPHIC_FILE_CACHE[cache_key] if $EGG_GRAPHIC_FILE_CACHE.has_key?(cache_key)

      species_data = self.get_species_form(species, form)
      if species_data.nil?
        $EGG_GRAPHIC_FILE_CACHE[cache_key] = nil
        return nil
      end
      if form > 0
        ret = pbResolveBitmap(sprintf("%s%s_%d%s", path, species_data.species, form, suffix))
        if ret
          $EGG_GRAPHIC_FILE_CACHE[cache_key] = ret
          return ret
        end
      end
    ...
    def self.check_cry_file(species, form, suffix = "")
      cache_key = [species, form, suffix]
      return $CRY_FILE_CACHE[cache_key] if $CRY_FILE_CACHE.has_key?(cache_key)

      species_data = self.get_species_form(species, form)
      if species_data.nil?
        $CRY_FILE_CACHE[cache_key] = nil
        return nil
      end
      if form > 0
        ret = sprintf("Cries/%s_%d%s", species_data.species, form, suffix)
        if pbResolveAudioSE(ret)
          $CRY_FILE_CACHE[cache_key] = ret
          return ret
        end
      end
      ret = sprintf("Cries/%s%s", species_data.species, suffix)
      res = (pbResolveAudioSE(ret)) ? ret : nil
      $CRY_FILE_CACHE[cache_key] = res
      return res
    end
