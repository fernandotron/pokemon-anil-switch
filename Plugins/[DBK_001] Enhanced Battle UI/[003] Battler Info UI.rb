...
class Battle::Scene
  ...
  def pbAddTypesDisplay(xpos, ypos, battler, poke)
    ...
    #---------------------------------------------------------------------------
    # Draws Tera type.
    if defined?(battler.tera?) && defined?(battler.pokemon.terastal_able?)
      if battler.tera?
        showTera = true
      else
  ...
  def pbAddEffectsDisplay(xpos, ypos, panelX, effects, idxEffect)
    ...
    textPos = []
    imagePos = [
      [@path + "info_effects", xpos + 240, ypos + 245],
    ]
    imagePos << [@path + "info_slider_base", panelX + 222, ypos + 132] if effects.length > 1
    #---------------------------------------------------------------------------
    # Draws the slider.
    #---------------------------------------------------------------------------
    if effects.length > 5
    ...
    drawFormattedTextEx(@enhancedUIOverlay, xpos + 246, ypos + 255, 208, desc, BASE_DARK, SHADOW_DARK, 18)
  end
  ...
  def pbGetDisplayEffects(battler)
    ...
    if battler.damageThreshold
      desc = _INTL("Los PS del Pokémon rival no bajarán del {1}% al ser atacado.", battler.damageThreshold.abs)
    ...
    if battler.hasActiveAbility?(:ACOMETIDA) && battler.turnCount == 0
      ...
      desc = _INTL("Sube su {1} en un {2}.", stat_name, percent)
      ...
      key_hash.each do |type, effects|
        effects.each do |effect|
          ...
          when :FocusEnergy
            ...
            desc = _INTL("Es más probable que el Pokémon realice ataques críticos.")
          #---------------------------------------------------------------------
          when :Attract
