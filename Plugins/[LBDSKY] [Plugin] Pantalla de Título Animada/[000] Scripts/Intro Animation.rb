...
class MTS_INTRO_ANIM2
  ...
  def initialize(viewport,sprites)
    ...
    for key in @scene.keys
      next if key == "logo" || key.include?('poke_sprite')
...
class MTS_INTRO_ANIM3
  ...
  def initialize(viewport,sprites)
    ...
    @sprites["saph"].bitmap = pbBitmap("Graphics/Titles/Intros/sapphire")
    @sprites["saph"].z = 900
...
class MTS_INTRO_ANIM6
  ...
  def initialize(viewport,sprites)
    ...
    @sprites["rays"].bitmap = pbBitmap("Graphics/Titles/Intros/rays")
    @sprites["rays"].center!(true)
    @sprites["rays"].opacity = 0
    @sprites["rays"].z = 920
    # letters
    @sprites["sil"] = Sprite.new(@viewport)
    @sprites["sil"].bitmap = pbBitmap("Graphics/Titles/Intros/letterSilhouette")
    @sprites["sil"].center!(true)
    @sprites["sil"].z = 900
    @sprites["sil"].angle -= 8 if $PokemonSystem.screensize < 2
    @sprites["let"] = Sprite.new(@viewport)
    @sprites["let"].bitmap = pbBitmap("Graphics/Titles/Intros/letter")
    ...
    for i in 0...136.delta(:add)
      ...
      for j in 0...24
        if @sprites["s#{j}"].opacity <= 0
          bmp = pbBitmap("Graphics/Titles/Particles/shine001")
