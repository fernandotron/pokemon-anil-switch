...
class Connection
  ...
  def self.open(host, port)
    ...
    begin
      ...
      raise Disconnected.new("connection timed out")
    rescue
    end
  end
...
class RecordParser
  ...
  def sym
    ...
    val = @fields.pop
    # Parche: Convertir símbolos inválidos conocidos a válidos
...
class RecordWriter
  ...
  def escape!(s)
    ...
    t.force_encoding("UTF-8") if t.respond_to?(:force_encoding)
    t.gsub!("\\", "\\\\")
    t.gsub!(",", "\,")
    return t
  end
