...
#    6 => [["Water rock", "Sea deep"], []]
  ...
  class TilesetBitmaps
    ...
    def add(filename)
      ...
      @bitmap_wraps[filename] = false
      if bitmap && bitmap.mega?
        log_compat("[TilesetBitmaps.add] Wrapping mega tileset: #{filename} (#{bitmap.width}x#{bitmap.height})...") rescue nil
        self[filename] = TilemapRenderer::TilesetWrapper.wrapTileset(bitmap)
        @bitmap_wraps[filename] = true
        bitmap.dispose
        log_compat("[TilesetBitmaps.add] Wrapped #{filename} OK: #{self[filename]&.width}x#{self[filename]&.height}") rescue nil
      else
    ...
    def set_src_rect(tile, tile_id)
      ...
      bmp = @bitmaps[tile.filename]
      return if !bmp || bmp.disposed?
      sx = ((tile_id - TILESET_START_ID) % TILESET_TILES_PER_ROW) * SOURCE_TILE_WIDTH
      sy = ((tile_id - TILESET_START_ID) / TILESET_TILES_PER_ROW) * SOURCE_TILE_HEIGHT
      if @bitmap_wraps[tile.filename]
        height = bmp.height
        col = (tile_id - TILESET_START_ID) * SOURCE_TILE_HEIGHT / (TILESET_TILES_PER_ROW * height)
        sx += col * TILESET_TILES_PER_ROW * SOURCE_TILE_WIDTH
        sy -= col * height
      end
      sx = [[sx, 0].max, [bmp.width - SOURCE_TILE_WIDTH, 0].max].min
      sy = [[sy, 0].max, [bmp.height - SOURCE_TILE_HEIGHT, 0].max].min
      tile.src_rect.set(sx, sy, SOURCE_TILE_WIDTH, SOURCE_TILE_HEIGHT)
    end
  ...
  class AutotileBitmaps < TilesetBitmaps
    ...
    def set_src_rect(tile, tile_id)
      ...
      if bmp.height == SOURCE_TILE_HEIGHT
        sx = frame * SOURCE_TILE_WIDTH
        sy = 0
        sx = [[sx, 0].max, [bmp.width - SOURCE_TILE_WIDTH, 0].max].min
        tile.src_rect.set(sx, sy, SOURCE_TILE_WIDTH, SOURCE_TILE_HEIGHT)
        return
      ...
      sx = 0
      sy = (tile_id % TILES_PER_AUTOTILE) * SOURCE_TILE_HEIGHT
      if wraps && high_id
        sx = SOURCE_TILE_WIDTH
        sy -= SOURCE_TILE_HEIGHT * TILES_PER_AUTOTILE / 2
      end
      sx += frame * SOURCE_TILE_WIDTH * (wraps ? 2 : 1)
      sx = [[sx, 0].max, [bmp.width - SOURCE_TILE_WIDTH, 0].max].min
      sy = [[sy, 0].max, [bmp.height - SOURCE_TILE_HEIGHT, 0].max].min
      tile.src_rect.set(sx, sy, SOURCE_TILE_WIDTH, SOURCE_TILE_HEIGHT)
    end
  ...
  class TileSprite < Sprite
    ...
    def set_bitmap(filename, tile_id, autotile, animated, priority, bitmap)
      self.bitmap       = bitmap
      self.src_rect.set(0, 0, SOURCE_TILE_WIDTH, SOURCE_TILE_HEIGHT) rescue nil
      self.zoom_x       = ZOOM_X
      self.zoom_y       = ZOOM_Y
      @filename         = filename
      @tile_id          = tile_id
  ...
  def update
    ...
    begin
      ...
      if @old_tone != @tone
        if @tiles
          @tiles.each do |col|
            ...
            col.each do |coord|
              next if !coord
              coord.each { |tile| tile.tone = @tone if tile }
            end
          end
        end
      ...
      if @old_color != @color
        if @tiles
          @tiles.each do |col|
            ...
            col.each do |coord|
              next if !coord
              coord.each { |tile| tile.color = @color if tile }
            end
          end
        end
      ...
      if $PokemonGlobal && $PokemonGlobal.respond_to?(:bridge) && $PokemonGlobal.bridge != @bridge
      ...
      if $map_factory && $map_factory.maps
        $map_factory.maps.each do |map|
          next unless map && map.data
          ...
          map_display_x_tile = map_display_x / DISPLAY_TILE_WIDTH rescue 0
          map_display_y_tile = map_display_y / DISPLAY_TILE_HEIGHT rescue 0
          start_x = [-map_display_x_tile, 0].max
          start_y = [-map_display_y_tile, 0].max
          end_x = @tiles_horizontal_count - 1
          end_x = [end_x, (map.width rescue 50) - map_display_x_tile - 1].min
          end_y = @tiles_vertical_count - 1
          end_y = [end_y, (map.height rescue 50) - map_display_y_tile - 1].min
