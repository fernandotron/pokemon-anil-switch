...
class PokemonTrainerCard_Scene
  ...
  def pbDrawTrainerCardFront
    ...
    textPositions = [
       ...
       [sprintf("%d", pbGet(POKEBATTLE_POINTS_VARIABLE)), 302 + 2, 118 + 32, 1, base, shadow],
    ...
    # @active_gamemodes = get_active_gamemodes
    # if !@active_gamemodes.empty? && @active_gamemodes.length > 2
    #   textPositions.push([_INTL("MODOS Activos: Pulsa la [Z]."), 32, 118 + 64, 0, base, shadow])
    # elsif !@active_gamemodes.empty? && @active_gamemodes.length <= 2
    #   textPositions.push([_INTL("MODOS:"), 32, 118 + 64, 0, base, shadow])
    #   textPositions.push([@active_gamemodes.join(", "), 302 + 2, 118 + 64, 1, base,shadow])
    # end
    @sprites["overlay"].z += 10
    pbDrawTextPositions(@overlay, textPositions)
    textPositions = [
      [_INTL("Pulsa [D] para girar la tarjeta."), 16, 70 + 280, 0, Color.new(216, 216, 216), Color.new(80, 80, 80)]
    ]
  ...
  def pbDrawTrainerCardBack
    ...
    textPositions = [
      ...
      [_INTL("Combates Online"), 32, 118 + 32 - 16, 0, base, shadow],
      [_INTL("{1}", $player.online_battles_wins), 302 + 2 + 48 - 2, 118 + 32 - 16, 1, base, shadow],
      [_INTL("{1}", $player.online_battles_lost), 302 + 2 + 48 + 63 *2, 118 + 32 - 16, 1, base, shadow],

      [_INTL("Intercambios Online"), 32, 118 + 32 - 16 + 32, 0, base, shadow],
      [_INTL("{1}", $player.online_trade_count), 44 + 111 * 2, 118 + 32 - 16 + 32, 1, base, shadow],
    ...
    8.times do |i|
      if $player.badges[i + 0 * 8]
        imagepos.push(["Graphics/UI/Trainer Card/badges0", x, 104 * 2+ 26, i * 48, 0 * 48, 48, 48])
      end
      x += 48 + 8
    end
  ...
  def pbTrainerCard
    ...
    loop do
      ...
      if Input.trigger?(Input::ACTION) && @active_gamemode && !@active_gamemodes.empty? && @active_gamemodes.length > 2
        while Input.press?(Input::ACTION)
          Graphics.update
          Input.update
        end
        pbMessage(_INTL("Modos Activos: {1}", @active_gamemodes.join(", ")))
      end
  ...
  def get_active_gamemodes
    active_gamemodes = []
    active_gamemodes << _INTL("Random") if defined?(RandomizedChallenge) && RandomizedChallenge.enabled?
    active_gamemodes << _INTL("Desafío") if defined?(ChallengeModes) && ChallengeModes.on?
    active_gamemodes << _INTL("Amarillo") if $game_switches[109]
    return active_gamemodes
  end
