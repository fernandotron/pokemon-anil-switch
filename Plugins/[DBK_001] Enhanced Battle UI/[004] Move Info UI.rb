...
class Battle::Scene  
  ...
  def pbUpdateMoveInfoWindow(battler, specialAction, cw)
    ...
    # Comprobar que sea terapagos y esté en forma 2
    terastal = true if battler.isSpecies?(:TERAPAGOS) && battler.form == 2
    #---------------------------------------------------------------------------
    ...
    displayAccuracy = (acc    == 0) ? "---" : acc.floor.to_s
    ...
    textPos.push(
      ...
      [_INTL("Pr:"),    xpos + 442, ypos + 40, :left,   BASE_LIGHT, SHADOW_LIGHT],
  ...
  def pbGetFinalModifiers(battler, move, type, baseDmg, power, acc, pri, chance, showTera)
    ...
    if battler.abilityActive?
      5.times do |i|
        ...
        when 1 # Abilities that alter additional effect chance.
          ...
          bonus = [_INTL("Efecto secundario aumentado por {1}.", battler.abilityName), BASE_RAISED, SHADOW_RAISED]
        #-----------------------------------------------------------------------
        when 2 # Abilities that alter priority.
          oldPri = pri
          pri = Battle::AbilityEffects.triggerPriorityChange(battler.ability, battler, move, pri)
          if pri > oldPri
            bonus = [_INTL("Prioridad aumentada por {1}.", battler.abilityName), BASE_RAISED, SHADOW_RAISED]
          elsif pri < oldPri
            bonus = [_INTL("Prioridad reducida por {1}.", battler.abilityName), BASE_LOWERED, SHADOW_LOWERED]
          end
        #-----------------------------------------------------------------------
        when 3 # Abilities that alter accuracy.
          ...
          if acc > oldAcc || acc == 0
            bonus = [_INTL("Precisión aumentada por {1}.", battler.abilityName), BASE_RAISED, SHADOW_RAISED]
          elsif acc < oldAcc
            bonus = [_INTL("Precisión reducida por {1}.", battler.abilityName), BASE_LOWERED, SHADOW_LOWERED]
          end
        #-----------------------------------------------------------------------
        when 4 # Abilities that alter base power.
          ...
          # power *= powMults[:attack_multiplier]
          if power > oldPower
            bonus = [_INTL("Poder potenciado por {1}.", battler.abilityName), BASE_RAISED, SHADOW_RAISED]
          elsif power < oldPower
            bonus = [_INTL("Poder debilitado por {1}.", battler.abilityName), BASE_LOWERED, SHADOW_LOWERED]
          end
        end
      end
    end
    ...
    if battler.item && battler.itemActive?
      if ![0, 100].include?(acc)  # Held items that alter accuracy.
        ...
        if acc != oldAcc
          if acc > oldAcc || acc == 0
            bonus = [_INTL("Precisión aumentada por {1}.", battler.itemName), BASE_RAISED, SHADOW_RAISED]
          elsif acc < oldAcc
            bonus = [_INTL("Precisión reducida por {1}.", battler.itemName), BASE_LOWERED, SHADOW_LOWERED]
          end
        end
      end
      if power > 0                # Held items that alter base power.
        ...
        power *= powMults[:power_multiplier]
        if power > oldPower
          bonus = [_INTL("Poder potenciado por {1}.", battler.itemName), BASE_RAISED, SHADOW_RAISED]
        elsif power < oldPower
          bonus = [_INTL("Poder reducido por {1}.", battler.itemName), BASE_LOWERED, SHADOW_LOWERED]
        end
      end
    end
    #---------------------------------------------------------------------------
