...
class Battle::Battler
  ...
  def pbCheckFormOnHeldItemChange
    ...
    if isSpecies?(:DIALGA)
      ...
      pbSimpleFormChange(newForm, _INTL("¡{1} se transformó!", pbThis))
    end
    #---------------------------------------------------------------------------
    ...
    if isSpecies?(:ARCEUS) && self.ability == :MULTITYPE
      ...
      pbSimpleFormChange(newForm, _INTL("¡{1} se transformó en tipo {2}!", pbThis, type.name))
    end
    #---------------------------------------------------------------------------
    ...
    if isSpecies?(:OGERPON)
      ...
      pbSimpleFormChange(newForm, _INTL("¡{1} se puso su {2}!", pbThis, maskName))
    end
  end
end
...
# Corrección de error.
class Battle::AI::AIBattler
  def has_active_item?(item = nil)
    return @battler.hasActiveItem?(item)
  end
end

class Battle::Battler
  alias has_active_item? hasActiveItem? unless method_defined?(:has_active_item?)
end