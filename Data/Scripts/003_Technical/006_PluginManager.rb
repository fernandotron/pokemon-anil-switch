...
module PluginManager
  ...
  def self.error(msg)
    log_compat("[PluginManager Warning] #{msg}") rescue puts("[PluginManager Warning] #{msg}")
  end
  ...
  def self.pluginErrorMsg(name, script)
    ...
    e.message.force_encoding(Encoding::UTF_8) if e && e.message
    message += e.message
  ...
  def self.needCompiling?(*args); false; end
  def self.compilePlugins(*args); false; end
  ...
  def self.runPlugins
    ...
    if !scripts || scripts.empty?
      log_compat("[PluginManager Warning] No se encontraron scripts en Data/PluginScripts.rxdata") rescue nil
      return
    end
    echoed_plugins = []
    skipped_plugins = 0
    total_loaded = 0
    scripts.each_with_index do |plugin, p_idx|
      name, meta, script = plugin
      next if !script || !meta
      if meta.key?(:disabled) && [true, 'true', 'verdadero', 'si', 'x'].include?(meta[:disabled].to_s.downcase)
        skipped_plugins += 1
        next
      end
    ...
    log_compat("[PluginManager] ¡Se cargaron #{scripts.length} plugins (#{total_loaded} scripts) exitosamente!") rescue puts("[PluginManager] ¡Se cargaron #{scripts.length} plugins exitosamente!")
  end
