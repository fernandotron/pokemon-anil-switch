...
module PokemonSprite::MosaicSprite
  ...
  def mosaicRefresh(bitmap)
    ...
    # Verificar si el bitmap está disposed antes de usarlo
    if !@oldbitmap || @oldbitmap.disposed?
      @inrefresh = false
      return
    end
    ...
    if @mosaic <= 0
      ...
      begin
        # Verificar nuevamente antes de acceder a width/height
        if @oldbitmap.disposed?
          @inrefresh = false
          return
        end
        ...
        # Limpiar bitmaps anteriores de forma segura
        if @mosaicbitmap && !@mosaicbitmap.disposed?
          @mosaicbitmap.dispose
        end
        @mosaicbitmap = nil
        
        if @mosaicbitmap2 && !@mosaicbitmap2.disposed?
          @mosaicbitmap2.dispose
        end
        @mosaicbitmap2 = nil
        
        # Crear nuevos bitmaps
        ...
        # Verificar una vez más antes de stretch_blt
        if @oldbitmap.disposed?
          @inrefresh = false
          return
        end
        ...
        # Si ocurre cualquier error RGSS (como disposed bitmap), limpiar y salir
        @mosaicbitmap&.dispose rescue nil
        @mosaicbitmap = nil
        @mosaicbitmap2&.dispose rescue nil
        @mosaicbitmap2 = nil
        @inrefresh = false
        return
      end
