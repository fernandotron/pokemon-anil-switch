...
class Battle::Scene::PokemonDataBox
  ...
  def refresh
    ...
    if @style
      # Verificar que el bitmap existe y no ha sido disposed
  ...
  def update_style
    ...
    if sideSize > @style.max_side_size && @style.id != :Basic
      @style = GameData::DataboxStyle.get(:Basic)
      set_style_properties(sideSize)
      # Verificar y liberar bitmap anterior
      if @databoxBitmap && !@databoxBitmap.disposed?
        @databoxBitmap.dispose
        @databoxBitmap = nil
      end
      ...
      # Verificar y liberar bitmap de HP anterior
      if @hpBarBitmap && !@hpBarBitmap.disposed?
        @hpBarBitmap.dispose
        @hpBarBitmap = nil
      end
      @hpBarBitmap = AnimatedBitmap.new(sprintf("%s/%s/overlay_hp%s", @path, @style.id, suffix))
      @hpBar.bitmap = @hpBarBitmap.bitmap if @hpBar
      @hpBar.src_rect.height = @hpBarBitmap.height / 3 if @hpBar
      @sprites["hpBar"] = @hpBar if @sprites && @hpBar
      # Verificar y liberar bitmap de contenido anterior
      if @contents && !@contents.disposed?
        @contents.dispose
      end
      @contents = Bitmap.new(@databoxBitmap.width, @databoxBitmap.height)
      self.bitmap = @contents
      pbSetSmallFont(self.bitmap) if self.bitmap
    end
  end
