...
module GameData
  ...
  class SpeciesMetrics
    ...
    def self.get_species_form(species, form, female = false)
      ...
      species = species.species if species.is_a?(GameData::Species)
  ...
  class Species
    ...
    def self.sprite_bitmap_from_pokemon(pkmn, back = false, species = nil)
      ...
      ret.compile_strip(pkmn, back) if ret
      return ret
    end
  end
end
...
class Pokemon
  def super_shiny_hue
    ...
    # Check if species has a specific hue defined in PBS
    ...
    if species_hue && species_hue != 0
      @cached_super_shiny_hue = species_hue
      return @cached_super_shiny_hue
    end
    
    # Generate and cache a consistent hue based on the species + form
    # This ensures all super shinies of the same species/form have the same color
    # Use the hash of the species symbol to get a consistent number
    species_symbol = GameData::Species.get_species_form(@species, @form).id
    species_hash = species_symbol.to_s.sum  # Sum of character codes for consistency
