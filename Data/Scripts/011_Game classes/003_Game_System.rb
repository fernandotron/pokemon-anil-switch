...
class Game_System
  ...
  def bgm_play_internal2(name, volume, pitch, position, track = nil) # :nodoc:
    ...
    begin
      ...
      Audio.bgm_play(name, vol, pitch || 100, 0, track)
    end
  end
  ...
  def bgm_play_internal(bgm, position, track = nil) # :nodoc:
    ...
    if bgm && bgm.name != ""
      bgm_name = bgm.name.to_s.start_with?("Audio/BGM/") ? bgm.name : ("Audio/BGM/" + bgm.name)
      bgm_play_internal2(bgm_name, bgm.volume, bgm.pitch, position || 0, track)
    else
      if !track || track == 0
        @bgm_position = position
        @playing_bgm = nil
