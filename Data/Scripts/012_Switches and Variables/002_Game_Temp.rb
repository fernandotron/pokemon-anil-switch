...
class Game_Temp
  ...
  attr_accessor :pokecenter_following_pkmn
  attr_accessor :no_follower_field_move
  attr_accessor :status_pulse
  attr_writer :followers

  def followers
    @followers = Game_FollowerFactory.new if !@followers && defined?(Game_FollowerFactory)
    return @followers
  end

  #-----------------------------------------------------------------------------
  # * Dynamic fallback for unknown plugin properties on Game_Temp
  #-----------------------------------------------------------------------------
  ...
  def initialize
    ...
    @mart_prices            = {}
    @bag_scene              = nil
    @pokecenter_following_pkmn = 0
    @no_follower_field_move = false
  end
