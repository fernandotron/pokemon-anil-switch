...
module MessageConfig
  ...
  FONT_SIZE                = 28
  FONT_Y_OFFSET            = 0
  SMALL_FONT_NAME          = "Power Clear"
  SMALL_FONT_SIZE          = 22
  SMALL_FONT_Y_OFFSET      = 0
  TINY_FONT_SIZE           = 16
  TINY_FONT_Y_OFFSET       = 0
  NARROW_FONT_NAME         = "Power Clear"
  NARROW_FONT_SIZE         = 28
...
def getSkinColor(windowskin, color, isDarkSkin, no_ctag = false)
  if !windowskin || windowskin.disposed? ||
    ...
    textcolors = [
      ...
      [176, 143,  93], [235, 235,  44],   # 6  Yellow
      ...
      [150, 150, 150], [40, 40, 40],    # 14  Black with white background
    ]
...
def pbSetSystemFont(bitmap)
  ...
  bitmap.font.name = MessageConfig.pbGetSystemFontName rescue nil
  bitmap.font.size = MessageConfig::FONT_SIZE rescue nil
  bitmap.text_offset_y = MessageConfig::FONT_Y_OFFSET rescue nil
end

# Sets a bitmap's font to the system small font.
def pbSetSmallFont(bitmap)
  return if !bitmap || bitmap.disposed?
  bitmap.font.name = MessageConfig.pbGetSmallFontName rescue nil
  bitmap.font.size = MessageConfig::SMALL_FONT_SIZE rescue nil
  bitmap.text_offset_y = MessageConfig::SMALL_FONT_Y_OFFSET rescue nil
end

def pbSetTinyFont(bitmap)
  return if !bitmap || bitmap.disposed?
  bitmap.font.name = MessageConfig.pbGetSmallFontName rescue nil
  bitmap.font.size = MessageConfig::TINY_FONT_SIZE rescue nil
  bitmap.text_offset_y = MessageConfig::TINY_FONT_Y_OFFSET rescue nil
end

# Sets a bitmap's font to the system narrow font.
def pbSetNarrowFont(bitmap)
  return if !bitmap || bitmap.disposed?
  bitmap.font.name = MessageConfig.pbGetNarrowFontName rescue nil
  bitmap.font.size = MessageConfig::NARROW_FONT_SIZE rescue nil
  bitmap.text_offset_y = MessageConfig::NARROW_FONT_Y_OFFSET rescue nil
end
...
# pbFadeOutInWhite(z) { block }
# Fades out the screen to white before a block is run and fades it back in after the
# block exits. z indicates the z-coordinate of the viewport used for this effect
def pbFadeOutInWhite(z = 99999, nofadeout = false)
  duration = 0.4   # In seconds
  col = Color.new(255, 255, 255, 0)  # Blanco en lugar de negro
  viewport = Viewport.new(0, 0, Graphics.width, Graphics.height)
  viewport.z = z
  timer_start = System.uptime
  loop do
    col.set(255, 255, 255, lerp(0, 255, duration, timer_start, System.uptime))  # Fade a blanco
    viewport.color = col
    Graphics.update
    Input.update
    break if col.alpha == 255
  ...
  begin
    ...
    if !nofadeout
      timer_start = System.uptime
      loop do
        col.set(255, 255, 255, lerp(255, 0, duration, timer_start, System.uptime))  # Fade desde blanco
        viewport.color = col
        Graphics.update
        Input.update
        break if col.alpha == 0
