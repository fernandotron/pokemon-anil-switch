...
class PokemonBagPartyPanel < Sprite
  ...
  def refresh
    ...
    if @helditemsprite&.visible && !@helditemsprite.disposed?
      ...
      @helditemsprite.y     = GameData::Item.try_get(@helditemsprite.item)&.is_mega_stone? ? self.y + 30 : self.y + 40
    ...
    if @refreshBitmap
      ...
      if !@pokemon.egg?
        if !@text || @text.length == 0
          ...
          # Draw exp share icon
          # if @pokemon.expshare && !@pokemon.egg? && !@pokemon.fainted? && @pokemon.status == :NONE
          #   path = File.join('Graphics', 'Pictures', 'expicon.png')
          #   pbDrawImagePositions(@overlaysprite.bitmap, [[path, 55, 25, 0, 0, 19, 17]])
          # end
          # Draw status
          status = -1
          if @pokemon.fainted?
...
class PokemonBag_Scene
  ...
  def pbRefreshIndexChanged
    ...
    if itemlist.item && GameData::Item.get(itemlist.item).is_machine?
      @sprites["itemtext"].text = GameData::Move.get(GameData::Item.get(itemlist.item).move).description
    else
      ...
      loop do
        ...
        if itemwindow.sorting
          ...
          if Input.repeat?(Input::UP) && thispocket.length   > 0 || 
            ...
            pbSEPlay("GUI sel cursor") if itemwindow.index != oldindex
          end
          # Change pockets
...
class PokemonBagScreen
  ...
  def pbStartScreen
    ...
    loop do
      ...
      if @bag.favourite?(item)
        commands[cmdFavourite = commands.length] = _INTL("No fav.")
      else
        commands[cmdFavourite = commands.length] = _INTL("Favorito")
      end
