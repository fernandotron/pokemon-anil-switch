  ...
  def setPokemon(list, page, gender = 0)
    PAGE_SIZE.times do |i|
      ...
      if species
        ...
        if gender.is_a?(Integer) && gender > 0
  ...
  def pbChooseDataList(cursor = nil)
    ...
    loop do
      ...
      if Input.repeat?(Input::UP)
        ...
        pbDrawDataNotes
        break
      elsif Input.trigger?(Input::SPECIAL) && cursor == :ability
        show_ability_diffs
      end
    end
  end
    ...
    if @api_data
      ...
      (1..3).each do |slot|
        ...
        if game_ability && api_ability
          ...
          game_key, game_data = game_ability
          slot_name = slot == 3 ? _INTL("Hab. Oculta") : _INTL("Hab. #{slot}")
      ...
      if !changed_abilities.empty? || !added_abilities.empty? || !removed_abilities.empty?
        ...
        removed_abilities.each do |slot_name, ability_name|
          diff_text += "\\c[2]#{_INTL("Eliminada")}: #{ability_name}\\c[0]\n"
        end
