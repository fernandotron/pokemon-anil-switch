    ...
    def update
      ...
      if $scene.is_a?(Scene_Map) && $scene.eye_of_truth_time > 0
        ...
        if !@eye_graphic || @eye_graphic.disposed?
          ...
          @eye_graphic.y = (Graphics.height / 2) - 38
        ...
        if !@mask || @mask.disposed?
          ...
          @mask.x = Graphics.width / 2 + 10
          @mask.y = (Graphics.height / 2) + 14
          @mask.opacity = 0
        end
        ...
        if @eye_graphic && !@eye_graphic.disposed? && @eye_graphic.opacity > 0
          @eye_graphic.angle += 0.5
          @eye_graphic.opacity -= 12.75 
          @mask.opacity -= 12.75 if @mask && !@mask.disposed?
          @effect.visible = false if @effect && !@effect.disposed?
          @effect.opacity = 255 if @effect && !@effect.disposed?
          @effect.zoom_x = @effect.zoom_y = 0 if @effect && !@effect.disposed?
        end
      end
    end
  end
...
def pbLensOfTruth
  if ($scene.eye_of_truth_time == 0)
    ...
    pbMessage(_INTL("El objeto ya está en uso."))
    return false
  end
end
...
ItemHandlers::UseInField.add(:LENSOFTRUTH, proc { |item|
  pbMessage(_INTL("¡\\PN usó la lente de la verdad!"))
  waves = []
  star = Sprite.new
  star.z = 2
  ...
  star.y = $game_player.screen_y - 38
