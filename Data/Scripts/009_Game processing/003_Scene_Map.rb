...
class Scene_Map
  ...
  def createSpritesets
    log_compat("[Scene_Map] createSpritesets: 1. TilemapRenderer.new...") rescue nil
    @map_renderer = TilemapRenderer.new(Spriteset_Map.viewport) if !@map_renderer || @map_renderer.disposed?
    log_compat("[Scene_Map] createSpritesets: 2. Spriteset_Global.new...") rescue nil
    @spritesetGlobal = Spriteset_Global.new if !@spritesetGlobal
    log_compat("[Scene_Map] createSpritesets: 3. Spriteset_Map.new for each map...") rescue nil
    @spritesets = {}
    $map_factory.maps.each do |map|
      log_compat("[Scene_Map]   -> Spriteset_Map.new for map #{map.map_id}...") rescue nil
      @spritesets[map.map_id] = Spriteset_Map.new(map)
    end
    log_compat("[Scene_Map] createSpritesets: 4. setSceneStarted...") rescue nil
    $map_factory.setSceneStarted(self)
    log_compat("[Scene_Map] createSpritesets: 5. updateSpritesets(true)...") rescue nil
    updateSpritesets(true)
    log_compat("[Scene_Map] createSpritesets completado con éxito.") rescue nil
  end
  ...
  def updateSpritesets(refresh = false)
    ...
    keys.each do |i|
      if $map_factory.hasMap?(i)
        @spritesets[i].update rescue nil
      else
        @spritesets[i]&.dispose rescue nil
        @spritesets[i] = nil
        @spritesets.delete(i)
      end
    end
    @spritesetGlobal.update rescue nil
    pbDayNightTint(@map_renderer) rescue nil
    @map_renderer.refresh if refresh
    @map_renderer.update rescue nil
    EventHandlers.trigger(:on_frame_update) rescue nil
  end
  ...
  def update
    ...
    if !pbMapInterpreterRunning? && !$PokemonGlobal.forced_movement?
      ...
      elsif (Input.respond_to?(:trigger_action?) && Input.trigger_action?(:menu))
        if !$game_system.menu_disabled && !$game_player.moving?
          $game_temp.menu_calling = true
          $game_temp.menu_beep = true
        end
      elsif (Input.respond_to?(:trigger_action?) && Input.trigger_action?(:shortcut)) || Input.trigger?(Input::SPECIAL)
  ...
  def main
    $scene = self
    log_compat("[Scene_Map.main] 1. createSpritesets...") rescue nil
    createSpritesets
    log_compat("[Scene_Map.main] 2. Graphics.transition...") rescue nil
    Graphics.transition
    log_compat("[Scene_Map.main] 3. Entrando al loop principal de Scene_Map...") rescue nil
    loop do
      Graphics.update
      Input.update
      update
    ...
    log_compat("[Scene_Map.main] 4. Saliendo del loop de Scene_Map...") rescue nil
    Graphics.freeze
    dispose
    if $game_temp.title_screen_calling
