...
module TrainerSensor
  ...
  def self.create(distance)
    ...
    if !@created
      ...
      @top.visible = true if @top && !@top.disposed?
      @bottom.visible = true if @bottom && !@bottom.disposed?
  ...
  def self.hide_immediate
    @triggered = false
    @created = false
    @top.visible = false if @top && !@top.disposed?
    @bottom.visible = false if @bottom && !@bottom.disposed?
    @top.y = -BAR_HEIGHT if @top && !@top.disposed?
    @bottom.y = Graphics.height + BAR_HEIGHT if @bottom && !@bottom.disposed?
  end
...
def update_trainer_bars
  if !$scene || !$scene.is_a?(Scene_Map) || !$game_map || !$game_map.events || !$game_player
    TrainerSensor.hide_immediate rescue nil
    return
  end
