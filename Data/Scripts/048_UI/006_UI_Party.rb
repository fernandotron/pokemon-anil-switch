...
class PokemonPartyPanel < Sprite
  ...
  def draw_shiny_icon
    ...
    if @pokemon.super_shiny?  
      pbDrawImagePositions(@overlaysprite.bitmap,
                          [["Graphics/UI/shiny_ur", 80, 48, 0, 0, 16, 16]])
    else
      pbDrawImagePositions(@overlaysprite.bitmap,
...
class PokemonParty_Scene
  ...
  def pbChangeSelection(key, currentsel)
    ...
    when Input::LEFT
      loop do
        currentsel -= 1
        break unless currentsel > 0 && currentsel < max_party_size && !@party[currentsel]
      end
    ...
    when Input::RIGHT
      loop do
        currentsel += 1
        break unless currentsel < max_party_size && !@party[currentsel]
      end
      if currentsel == numsprites
        currentsel = (@party.length == 0) ? max_party_size : 0
      end
    when Input::UP
      if currentsel >= max_party_size
        currentsel -= 1
        while currentsel > 0 && currentsel < max_party_size && !@party[currentsel]
          currentsel -= 1
        end
        currentsel = numsprites - 1 if currentsel < max_party_size && currentsel >= @party.length
      else
        loop do
          currentsel -= 2
  ...
  def pbHardRefresh
    ...
    lastselected = max_party_size if lastselected < 0
    # Settings::MAX_PARTY_SIZE.times do |i|
    max_party_size.times do |i|
      if @party[i]
        @sprites["pokemon#{i}"] = PokemonPartyPanel.new(@party[i], i, @viewport)
